...
``qwen3_vl`` 安装路径同时包含 Hugging Face 和 SGLang reward runtime。
随后在 reward 配置中使用 ``history_vlm`` 或 ``history_vlm_sglang``。
QwenTrend 示例使用
...
如果使用 SGLang 后端，保持相同的 history-buffer reward 配置，并设置
``model_type: "history_vlm_sglang"``。该后端复用相同的 input builder 和
reward parser，但会把渲染后的 Qwen3-VL prompt 与视频帧直接发送给进程内
SGLang engine。
