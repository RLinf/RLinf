...
4. 可选 EDAC Critic Diversity 正则
-----------------------------------

SAC 提供了经过单元测试的 EDAC-style critic diversity loss helper，可用于
SAC-N/RLPD 风格 critic ensemble 的实验。该正则项会计算每个 Q head 对 action
的梯度，并惩罚 critic ensemble 中方向过于一致的 action-gradient。

该选项默认关闭；当 ``edac_eta: 0.0`` 或没有配置该字段时，SAC 行为与原来一致。
启用 EDAC 需要额外的 critic action-gradient autograd graph，因此会增加显存和计算开销。
当前 FSDP SAC/RLPD worker 尚未把该 helper 接入 critic update path，正数
``edac_eta`` 会在 config validation 阶段提前失败。如果要直接实验该 helper，
需要在自定义 worker 中 import
