...
本文介绍 RLinf 内置的性能分析工具：

- **Nsight Systems**：基于 ``cluster.nsight`` 配置，通过 ``nsys profile``
  对指定 Ray worker group 做系统级 GPU/CPU profiling。
- **分布式追踪**：基于 ``runner.tracer`` 配置的轻量级 HTTP 分布式追踪系统，
  记录 runner 和所有 worker 跨节点的执行时间线。

Nsight Systems
------------------------------

借助这套机制，你可以对指定 Ray worker group 执行 ``nsys profile`` 包装，
采集 CUDA kernel、cuDNN、cuBLAS、NVTX，以及可选的 CPU runtime 相关时间线。
...
RLinf 集成了一个轻量级的基于 HTTP 的分布式追踪系统。它可以在不拖慢训练主循环的
前提下，记录 runner 和所有 Ray worker（环境、Rollout、Actor 等）跨节点的执行
时间线，并导出为 Chrome Trace 格式做交互式可视化。
