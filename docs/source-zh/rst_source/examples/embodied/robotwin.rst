...
在运行π\ :sub:`0`\评估前，需要准备对应的环境资产与归一化统计文件，目录结构如下：

.. code-block:: text

   pi0_base/
   ├── assets/                               # 环境资产文件
   └── robotwin/repo_id/norm_stats.json      # RoboTwin 任务的状态归一化统计数据

norm_stats.json 用于对机器人状态与动作进行归一化处理，是 π₀ 模型推理过程中的必要文件。
如果目标任务缺少对应的 norm_stats.json 文件，请参考
RoboTwin π\ :sub:`0`\使用文档 <https://robotwin-platform.github.io/doc/usage/Pi0.html#5-5-finetune-model>
生成，并将其放置在 **robotwin/repo_id** 目录下。
