...
.. note::
   ...
   - 独立的 RLinf 容器只负责 RLinf 运行时与 Ray worker，不直接接管 ``roscore``、UI 或 ``run.sh``。
...
在只做策略 rollout 或真机评估时，RLinf 提供了通用 Turtle2 部署环境，
注册名为 ``Turtle2DeployEnv-v1``。对应的环境片段配置为：

.. code-block:: text

   examples/embodiment/config/env/realworld_turtle2_deploy.yaml

可以将它作为 eval-only 配置中的 ``env.eval`` 使用。例如复制
``examples/embodiment/config/realworld_eval.yaml`` 后，将 env default 替换为：
...
Turtle2 deploy 环境片段会将 ``init_params.id`` 设置为
``Turtle2DeployEnv-v1``，并把 ``action_mode`` 放在 ``override_cfg`` 中。
默认保持 Turtle2 原有的 relative-pose 动作语义：
...
``create_turtle2_deploy_env()`` 会在 Turtle2 本地组装 wrapper stack：

- ``relative_pose`` 下可选启用 ``DualRelativeFrame``，把末端坐标系里的
  增量动作转换回机器人基坐标系。
- ``use_master_takeover=True`` 时会注入 ``MasterTakeoverIntervention``；
  如果 ``keyboard_running_mode`` 也启用，则继续挂上 ``KeyboardRunningModeWrapper``，
  然后再接 reward wrapper。
- 两种部署模式最后都会经过 ``DualQuat2EulerWrapper``，因此策略看到的是欧拉角
  格式的 TCP 状态。
- 只有当策略输出绝对位姿命令时，才显式设置 ``override_cfg.action_mode: absolute_pose``。
