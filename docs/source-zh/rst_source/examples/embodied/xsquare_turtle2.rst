...
``Turtle2DeployEnv-v1``，并在 env 层暴露 ``action_mode``。默认保持
Turtle2 原有的 relative-pose 动作语义：
...
- ``relative_pose`` 是默认模式，使用 ``DualRelativePoseActionWrapper``。默认还会启用
  ``DualRelativeFrame``，将末端坐标系下的增量动作转换回机器人基坐标系。
- 只有当策略输出绝对位姿命令时，才显式设置 ``action_mode: absolute_pose``。
- 两种模式最后都会经过 ``DualQuat2EulerWrapper``，因此策略看到的是欧拉角格式
  的 TCP 状态。
