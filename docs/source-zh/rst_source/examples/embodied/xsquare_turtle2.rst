...
``Turtle2DeployEnv-v1``。通过 ``override_cfg.action_mode`` 选择 Turtle2
动作合同，默认保持 Turtle2 原有的 relative-pose 动作语义。顶层
``action_mode`` 仍会为了兼容旧配置被接受，但新配置建议写在
``override_cfg.action_mode``：
