...
.. code-block:: text
   ...
   │                      │     │                      │     │                        │     │                      │
   │  对每条轨迹计算      │     │  训练价值模型学习     │     │  使用价值模型计算每个   │     │  以优势为条件进行     │
   │  折扣累积回报        │     │  回报预测             │     │  时间步的优势           │     │  CFG 策略训练         │
   └──────────────────────┘     └──────────────────────┘     └────────────────────────┘     └──────────────────────┘
...
配置文件位于 ``examples/process/config/compute_returns.yaml``：
...
配置文件位于 ``examples/sft/config/libero_cfg_openpi.yaml``：
...
Step 3 完成后，可以使用 ``examples/process/visualize_advantage_dataset.py`` 对优势分布进行可视化分析，
