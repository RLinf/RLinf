...
通过该项优化，DreamZero 5B 和 14B 模型在不改变 ``mbs=1``（每 GPU 微批大小，下同）的配置下，分别获得 :strong:`50%`（从 1.8s/step 降到 1.2s/step）和 :strong:`34%`（从 9s/step 降到 6.7s/step）的训练加速。
...
以下所有测试均在 :strong:`Droid 数据集`（单样本含左、右、腕部三个视角，视频规格 33 frames × 480 × 640）上，使用 8×H100 GPU 完成。
...
5B 模型使用 GBS=256 测试。FSDP2 Base 版本由于 PyTorch 的一些限制不能开大 MBS，导致吞吐受限；RLinf 解决了这些问题并取得了显著的吞吐增长。训练吞吐从官方代码的 1.1 samples/sec/gpu 飙升至 :strong:`4.44 samples/sec/gpu`，实现了约 :strong:`4 倍`的训练加速。
