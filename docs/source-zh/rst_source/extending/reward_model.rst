...
默认 checkpoint 为 ``0, 20, ..., 200``。step 0 使用随机初始化策略，step 20--120
使用 early 模板，step 140--200 使用 late 模板。每个 checkpoint 固定使用 seed 0，
并由一个四卡 Ray eval 任务完成采集。采集使用 ``simple`` observation、忽略提前终止，
并固定运行 50 个环境步，避免提前成功被错误保存为短失败样本。需要重采指定
checkpoint 时，
使用相同采集命令并增加例如 ``STEPS="80 120 160"``；同名 episode 会被覆盖。
...
预处理首先按 source episode 划分数据；如果任何 episode 同时出现在 train 和 eval，脚本会直接报错退出。
默认的 online-aligned 模式在 action 后 observation 索引 ``5, 10, ..., 50`` 各生成一个 5 帧窗口。索引 0 是 reset observation，不会作为第一个在线窗口。每个窗口直接使用 ``infos[end_step]["success"]`` 标注；保留自然类别比例，不添加 near-terminal 正样本或采样 hard negative。调试时可通过 ``UNIFORM_STEPS`` 只处理部分 checkpoint；默认仍处理 0 到 200。经校验的 step-100 pilot 生成训练集 2,700/6,560 个正/负样本，以及验证集 221/759 个正/负样本。
manifest 直接引用原始 episode pickle 和帧范围，不复制图像。只加载可信 pickle，
因为 Python 反序列化 pickle 时可能执行代码。

3. 使用验证过的 LoRA 配置微调 Qwen3-VL-4B（稀疏 success）：
...
脚本使用 LoRA rank 16、学习率 ``1e-5``、micro batch size 4、global batch size 256、
``format_ce_coef=2``、按类别比例加权的 success loss 和 400 个优化步；每 100 步
评估并保存。分别检查 positive recall 和 negative accuracy，并选择 balanced accuracy
最高的 checkpoint；不要只看整体 accuracy。

4. 构建稠密 potential 数据（state-value teacher + potential/progress 窗口）：
...
脚本会把选出的 checkpoint 路径写入 ``${OUTPUT_ROOT}/SELECTED_CKPT.txt``
（优先 ``global_step_${MAX_STEPS}``，否则取最大的 ``global_step_*``）。
后续步骤用该文件，无需手写 checkpoint 目录。

6. 抽取特征并训练 scalar potential head：
...
在线推理与 SFT 使用完全相同的 prompt 和 ``0`` / ``1`` 生成协议。potential
checkpoint 和 scalar head 生成受限的 dense shaping，独立训练的 success LoRA
生成稀疏 success 信号。生成 ``1`` 时只发放一次 ``+1`` bonus；``0`` 和非法输出
不加分。环境 reward 被关闭，VLM 每 5 步推理一次，训练固定运行 50 步以保持和
预处理一致。


PPO reward 由受限的 Qwen potential difference 和生成式 success bonus 共同组成。
续训时把 ``RESUME_DIR`` 设为已保存的 ``checkpoints/global_step_N``，并把
``MAX_STEPS`` 设为目标总步数；例如从 step 60 续训时设为 160。
启动脚本每 5 步评估、每 20 步保存，并默认把 actor、rollout、环境和 reward
都放置到 GPU 0-3。只有硬件布局不同时才需要覆盖 ``PLACEMENT``。
在线 Qwen micro-batch 默认为 32。启动脚本使用 main 分支支持的
``history_buffer`` 分发模式；二分类 parser 仍只把生成的 ``1`` 映射为
``+1``，所以实际 reward 仍是稀疏 reward。
