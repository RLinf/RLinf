...
完整成功 episode 的最后 5 帧窗口标为 ``1``；完整失败 episode 提供一个终局 ``0``，
完整和不完整 episode 都可以提供最多三个非终局 hard-negative ``0``。成功 episode 的
hard negative 至少早于最终窗口 8 步。确定性的源 episode 划分和全局随机数顺序可复现
已验证的 119 数据集：训练集正/负样本为 3,673/11,019，验证集为 405/1,215。manifest
直接引用原始 episode pickle 和帧范围，不复制图像。只加载可信 pickle，因为 Python
反序列化 pickle 时可能执行代码。
