...
1. Clone RLinf & One-Click Environment Setup
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

First, clone the RLinf repository and use ``install.sh`` to configure the underlying environment specifically for LingBot-VLA (the script will automatically pull the LingBot source code into the ``.venv/lingbot-vla`` directory and resolve all high-risk dependency conflicts):
...
.. code-block:: bash
    ...
    # One-click installation of LingBot-VLA native environment and RoboTwin base dependencies
...
.. code-block:: bash
    ...
    # 1. Download LingBot 4B base model weights
    python3 scripts/download_hf_model.py --repo_id robbyant/lingbot-vla-4b --local_dir lingbot-vla-4b

    # [Important Path Fix] Eliminate the nested folder trap generated by the download script
    cd lingbot-vla-4b
    mv lingbot-vla-4b/* .
    rmdir lingbot-vla-4b
    cd ..

    # 2. Download Qwen foundation weights
