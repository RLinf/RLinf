...
Below we break down the three core optimization techniques that make this possible.


Operator and Compute Graph Optimization: Torch Compile + CUDA Graph
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Python-level operator and scheduling overhead is often the "hidden killer" of GPU peak performance. RLinf deeply integrates ``torch.compile`` and CUDA Graph:

- **Torch Compile**: through low-level compilation, performs deep kernel fusion on inefficient operators in the Diffusion architecture, including WanRMSNorm and adaLN-zero.
- **CUDA Graph**: captures and replays the compute graph, eliminating the CPU scheduling bottleneck of GPU kernel launches. In DreamZero training, the CausalWanSelfAttention component has particularly dense kernel launches, where CUDA Graph yields effective speedups.

With this optimization alone, DreamZero 5B and 14B models (at ``mbs=1``, microbatch size per GPU) achieve **50%** (1.8s/step → 1.2s/step) and **34%** (9s/step → 6.7s/step) training acceleration respectively.


FSDP2 Parallel Optimization: Unlocking Arbitrary mbs and Recompute
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Supporting arbitrary microbatch sizes, FSDP2 (Fully Sharded Data Parallel 2) parameter tuning, and recompute (activation checkpointing) are essential performance-tuning tools in large-model training. However, the official DreamZero baseline had significant engineering limitations—defaulting to DeepSpeed ZeRO2-offload, running the image encoder sample-by-sample without batching, etc.—which severely constrained the tuning space.

RLinf rebuilt the engineering foundation, delivering a robust and highly configurable tuning matrix:

**Stable FSDP2 support**
  FSDP2 is PyTorch's latest ZeRO implementation and RLinf's default parallelism strategy for mid-scale models. The original DeepSpeed-based approach had inherent limitations: a compatibility conflict between ZeRO3 and the causal-conv state maintenance in the VAE module forced developers back to the lower-performance ZeRO2-offload mode. Additionally, DeepSpeed's post-backward hooks incurred high CPU-side overhead, limiting overall training throughput. By migrating to the FSDP2 backend, RLinf resolved these architectural conflicts and bottlenecks. Users can now freely switch between sharding strategies based on memory requirements while maintaining stability and efficiency.

**Flexible microbatch sizes**
  In early FSDP2-enabled DreamZero training, combinations of mbs > 1, recompute, and FSDP2 would trigger complex low-level compute graph conflicts, and the unbatchable image encoder ate into the gains from larger mbs. RLinf resolved these incompatibilities through engineering effort and made the image encoder batch-efficient. This allows users to freely configure any mbs, enabling fine-grained tuning based on GPU memory headroom and compute throughput needs. For example, on DreamZero 5B training without recompute, raising mbs from 1 to 2 leaves step time nearly unchanged (1.2s/step → 1.3s/step) while increasing throughput by **85%**.

**Recompute deeply co-optimized with accelerated operators**
  RLinf achieved stable decoupling and cooperation between recompute (activation checkpointing), CUDA Graph, and FSDP2—overcoming compatibility limitations in the native PyTorch framework. This turns recompute into a reliable, quantifiable tuning dimension. In memory-constrained environments, the system trades a small amount of compute time for substantial memory savings, enabling much larger mbs and significantly higher overall throughput. On DreamZero 5B: without recompute, per-GPU mbs is capped at 2, reaching ~1.2s/step (1.7 samples/sec/gpu); with recompute, per-GPU mbs can go to 32, achieving 7.2s/step (4.4 samples/sec/gpu)—a **158% throughput improvement under the same hardware**.

Through the combined tuning of FSDP2, mbs, and recompute, RLinf further improves DreamZero 5B training performance by **266%** on top of the first optimization (from 1.2 samples/sec/gpu to 4.4 samples/sec/gpu).


Video Data Pipeline: Breaking the I/O Throughput Bottleneck
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

As compute density rises dramatically (from the two optimizations above), data loading becomes the new bottleneck. In DreamZero training, video decoding and preprocessing are extremely CPU-intensive—multi-view scenarios require processing left, right, and wrist camera streams simultaneously. The traditional PyAV decoder cannot sustain high-frequency throughput, and naively adding ``num_workers`` only makes things worse (excess data-loader processes compete for CPU, slowing down GPU kernel dispatch from the main training thread).

To find the sweet spot between decode speed and system resource overhead, the RLinf team benchmarked mainstream video processing libraries:
...
.. list-table::
   ...
   * - Decode back end
     - Avg. per-video decode time
     - Resource footprint
   * - PyAV
     - 816 ms
     - High (heavy resource usage, slowest)
   * - Decord
     - 435 ms
     - High (large transient load spikes)
   * - Torchcodec
     - 446 ms
     - Low (stable CPU utilization curve)
...
Compared to the native PyAV solution, per-video decode time drops by nearly **400ms**. In DreamZero's multi-view training (left, right, and wrist camera videos), cumulative decode savings reach ~**1.2s**—providing ample data "ammunition" to further push GPU compute utilization.
