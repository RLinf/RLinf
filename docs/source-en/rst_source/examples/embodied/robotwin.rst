...
Before running π₀ evaluation, the required environment assets and normalization statistics must be prepared with the following directory structure:

.. code-block:: text

   pi0_base/
   ├── assets/                               # RoboTwin environment assets
   └── robotwin/repo_id/norm_stats.json      # State normalization statistics for RoboTwin tasks

The norm_stats.json file is used to normalize robot states and actions and is required during π₀ inference.

If the target task does not provide a corresponding norm_stats.json file, please refer to the
RoboTwin π\ :sub:`0`\ usage documentation <https://robotwin-platform.github.io/doc/usage/Pi0.html#5-5-finetune-model>
to generate the normalization statistics, then copy the file to the **robotwin/repo_id** directory.
