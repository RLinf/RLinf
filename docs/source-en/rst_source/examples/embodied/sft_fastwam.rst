...
Diagnose Resource Loading Errors
--------------------------------

If SFT or evaluation fails during initialization, check the FastWAM resource
messages before the traceback. Model loading reports the checkpoint, normalization
statistics and the local paths used by the upstream component loader. A missing
VAE, T5 encoder, tokenizer or Wan DiT produces a warning with the repository,
``DIFFSYNTH_DOWNLOAD_SOURCE`` and ``DIFFSYNTH_SKIP_DOWNLOAD``. Automatic downloading
remains enabled unless you disable it; a later HTTP error does not mean the local
resource was present. Compare the reported path with your files and
``DIFFSYNTH_MODEL_BASE_PATH``. These checks report local matches, not weight integrity.

SFT also reports dataset directories, ``data.text_embedding_cache_dir`` and the
normalization statistics path. Each dataset path must contain ``meta/info.json``;
the text cache must contain files produced by ``scripts/precompute_text_embeds.py``.
Missing paths raise an error before dataset construction. Individual missing cache
entries or dataset files can still fail when samples are read. If initialization
fails inside FastWAM, the original traceback is retained alongside the resource
context. These messages come from Python; a workflow shell check that exits before
Python starts will not emit them.
