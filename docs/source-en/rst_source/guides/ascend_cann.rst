Run RLinf on Ascend CANN
========================

Run RLinf examples on Ascend CANN. This page covers CANN-specific dependency
installation and runtime access to the host Ascend driver. Task descriptions,
algorithm details, model downloads, configuration files, metrics, and results
remain specific to each example.
...
For Agentic RL examples, install the Agentic stack instead:
...
Agentic RL on Ascend
--------------------

The following FSDP2 and SGLang recipes run Agentic RL workloads on Ascend. They
use the same task-specific datasets and models as their NVIDIA counterparts.

SearchR1
~~~~~~~~

SearchR1 requires a local Qdrant wiki retrieval server. Download the SearchR1
data and the ``e5-base-v2`` retriever as described in
:doc:`the SearchR1 guide <../examples/agentic/searchr1>`, set the paths in the
two scripts below, and start the index builder and server in separate terminals:
...
Download the ``calc_x`` dataset as described in
:doc:`the calc_x guide <../examples/agentic/agentlightning_calc_x>`, set the
model and dataset paths in the Ascend config, and run:

.. code-block:: bash

   bash examples/agent/agentlightning/calc_x/run_calc_x.sh qwen2.5-1.5b-enginehttp-multiturn-fsdp_ascend

Coding RL with an LLM judge
~~~~~~~~~~~~~~~~~~~~~~~~~~~

The offline LLM-judge recipe needs the Code-FIM dataset, a Qwen2.5-Coder model,
and credentials for the judge endpoint. Export the endpoint settings before
launching the Ascend config:
...
LIBERO settings that stay the same
----------------------------------
