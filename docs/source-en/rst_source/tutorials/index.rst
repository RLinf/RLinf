...
- **Main Process.** The middle part of the figure provides a more detailed breakdown of the main function. Here, Worker placement is determined by `Worker Placement Strategy`, and WorkerGroups (e.g., ActorGroup, RolloutGroup) are created via `create_group()`. The Runner, defined in `rlinf/runners/`, receives these WorkerGroups and establishes communication Channels (`Channel.Create()`) for inter-group data exchange. Each WorkerGroup executes its core task (e.g., `train`, `rollout`) and communicate through the assigned Channels. After each iteration, ActorGroup synchronizes its model weights to the RolloutGroup using `sync_model`.
