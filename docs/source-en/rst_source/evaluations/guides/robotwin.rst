...
.. list-table::
   ...
   * - ``robotwin_<task>_openwam_eval.yaml``
     - all 50 RoboTwin 2.0 tasks (generated by ``toolkits/openwam/gen_robotwin_eval_recipes.py``)
     - OpenWAM
...
- ``rollout.model.num_action_chunks: 32`` with ``openwam.inference_horizon: null``: whole 32-step chunks, like OpenWAM's own RoboTwin client; ``max_episode_steps`` and ``step_lim`` are RoboTwin's ``_eval_step_limit.yml`` budget rounded up to a multiple of 32 (e.g. click_bell 400 -> 416, place_empty_cup 500 -> 512); all tasks share ``env/robotwin_openwam_aloha.yaml``, and tasks without RLinf eval seeds fall back to random seeds
- The training-time prompt prefix is applied automatically; nothing to configure
- Every end-effector target is planned individually through RoboTwin's ``take_action(..., action_type="ee")`` (mplib), about 25 to 30 s per 32-step chunk; the sub-environments of one env worker share a lock, so more environments per env GPU only serialize and the recipes keep 2, scaling with ``rollout_epoch`` or more env GPUs instead; ``env.eval.robotwin_step_timeout_s`` (1800 s in the preset) bounds each chunk wait; SAPIEN needs a ``DISPLAY`` (start ``Xvfb :99`` on headless nodes) and the sapien/mplib patches from ``install_robotwin_env`` in ``requirements/install.sh``
- ``env.eval.instruction_type: unseen`` samples each episode's instruction from RoboTwin's unseen pool, as OpenWAM's own RoboTwin protocol does; RoboTwin's default (and every other RLinf RoboTwin recipe) is ``seen``
