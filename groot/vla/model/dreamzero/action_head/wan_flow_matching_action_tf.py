...
def get_local_wan_dir() -> str | None:
    """Check WAN_CKPT_DIR env var for a local Wan checkpoint directory."""
    local_wan_dir = os.environ.get("WAN_CKPT_DIR")
...
def ensure_file(path: str | None, hf_filename: str, repo_id: str = WAN_HF_REPO_ID) -> str:
    """Return a valid local path: use `path` if it exists, try local Wan dir, then HuggingFace."""
...
class WANPolicyHeadConfig(PretrainedConfig):
    ...
    action_loss_scale: float = field(default=1.0, metadata={"help": "Multiplier for action loss relative to dynamics loss."})
    ...
    def __init__(
        ...
        if not config.skip_component_loading:
            ...
            if dit_dir is None or not os.path.isdir(dit_dir):
                local_wan_dir = get_local_wan_dir()
                if local_wan_dir is not None:
                    local_index = os.path.join(local_wan_dir, "diffusion_pytorch_model.safetensors.index.json")
                    local_single = os.path.join(local_wan_dir, "diffusion_pytorch_model.safetensors")
                    if os.path.exists(local_index) or os.path.exists(local_single):
                        logger.info("Using local Wan diffusion directory: %s", local_wan_dir)
                        dit_dir = local_wan_dir
                if dit_dir is None or not os.path.isdir(dit_dir):
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
            ...
            if actions.numel() > 0:
                ...
                loss = weighted_dynamics_loss + self.config.action_loss_scale * weighted_action_loss
            else:
