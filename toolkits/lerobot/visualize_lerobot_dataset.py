...
def _resolve_parquet_files(dataset_path: Path) -> list[Path]:
    """Find all episode parquet files under ``dataset_path``.

    Handles three layouts:

    * a single parquet file (returned as-is);
    * a single dataset root containing ``data/`` and ``meta/``;
    * a tree containing many shards — each shard has its own ``data/`` +
      ``meta/`` (e.g. ``rank_0/id_0/data/...``, ``rank_0/id_1/data/...``).

    LeRobot v3 names episode parquets ``file-NNN.parquet`` and writes
    metadata parquets under ``meta/`` (``tasks.parquet``,
    ``episodes.parquet``).  Always exclude ``meta/`` from the result.
    """
    ...
    files = [
        p for p in dataset_path.glob("**/data/chunk-*/*.parquet") if p.is_file()
    ]
    return sorted(files)


def _shard_root_for(parquet_path: Path) -> Path:
    """Return the dataset root for an episode parquet.

    Episode parquets live at ``<root>/data/chunk-NNN/file-NNN.parquet``,
    so the root is two directories above the file's parent.
    """
...
def _infer_episode_index(parquet_path: Path, rows: list[dict[str, Any]]) -> int:
    ...
    match = re.search(r"(?:episode|file)_(\d+)\.parquet$", parquet_path.name)
    if match:
...
def main() -> None:
    ...
    # Cache per-shard meta so we read each shard's info/episodes/tasks
    # exactly once even when many parquet files belong to the same shard.
    meta_cache: dict[Path, tuple[dict[str, Any], dict[int, dict[str, Any]], dict[int, str]]] = {}

    def _shard_meta(shard_root: Path):
        if shard_root not in meta_cache:
            meta_dir = shard_root / "meta"
            meta_cache[shard_root] = (
                _load_json(meta_dir / "info.json"),
                _build_episode_meta_map(meta_dir),
                _build_task_map(meta_dir),
            )
        return meta_cache[shard_root]

    for parquet_path in parquet_files:
        if parquet_path.is_file() and parquet_path.parent.name.startswith("chunk-"):
            shard_root = _shard_root_for(parquet_path)
        else:
            shard_root = dataset_path if dataset_path.is_dir() else dataset_path.parents[2]

        dataset_info, episode_meta_map, task_map = _shard_meta(shard_root)
        # Label the output folder with the shard's path relative to the
        # input dir (e.g. ``rank_0_id_0``) so episodes from different
        # shards never collide.
        try:
            rel = shard_root.relative_to(dataset_path)
            shard_label = "_".join(rel.parts) if rel.parts else ""
        except ValueError:
            shard_label = shard_root.name
        ...
        print(
            f"Exported episode {episode_index:06d} from "
            f"{shard_label or '.'}/{parquet_path.name}"
        )
