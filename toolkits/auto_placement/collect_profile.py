...
def main(cfg, step_per_env: int = 1) -> None:
    ...
    if not has_target_env_num(cfg):
        ...
        for env_num in env_num_test_list:
            ...
            logger.info(
                f"Profiled with env_num={env_num}, avg_env_cost={collector.env_cost}, avg_rollout_cost={collector.rollout_cost}, avg_actor_cost={collector.actor_cost}"
            )
    ...
    logger.info(
        "Next Step: use profile data to run the auto placement script to find the best placement strategy for your task"
    )
    logger.info(
        "[Usage] bash examples/embodiment/run_placement_autotune.sh <your_config_name>"
    )
