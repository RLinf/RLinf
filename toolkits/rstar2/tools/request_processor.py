...
class RequestProcessor:
    ...
    def __init__(
        ...
        concurrency: int,
        batch_submit_func: BatchSubmitFunc,
    ):
        """
        ...
        if batch_timeout_seconds <= 0:
            print(
                "Warning: batch_timeout_seconds <= 0,"
                + " batching will be strictly based on batch_size or queue availability."
            )
        ...
        print(
            f"[{time.monotonic():.4f}] RequestProcessor initialized with"
            + f"concurrency={self._concurrency} (async thread)."
        )
    ...
    async def send_request(self, request_payload: Any, timeout: float = None):
        ...
        self._pending_requests[request_id] = {"future": future, "payload": request_payload}
    ...
    async def stop(self):
        ...
        wait_tasks = [
            asyncio.create_task(req_info["future"])
        ...
        if wait_tasks:
            ...
            try:
                await asyncio.wait_for(
                    asyncio.gather(*wait_tasks, return_exceptions=True), timeout=stop_results_timeout
                )
        ...
        if self._pending_requests:
            print(
                f"[{time.monotonic():.4f}] Warning: Stopping with {len(self._pending_requests)}"
                + " requests still pending (futures not completed/timed out)!"
            )
    ...
    async def _sender_worker(self):
        ...
        try:
            while self._running or not self._submission_queue.empty():
                ...
                try:
                    first_item_id = await asyncio.wait_for(
                        self._submission_queue.get(), timeout=batch_gathering_timeout
                    )
                    ...
                    if not batch_item_ids:
                        continue
                    print(
                        f"[{time.monotonic():.4f}] Worker: Timeout,"
                        + f" but gathered {len(batch_item_ids)} items. Proceeding to send."
                    )
                    pass
    ...
    async def _perform_send_batch(self, batch_item_ids: list[str]):
        ...
        try:
            ...
            if len(results_list) != len(batch_info):
                print(
                    f"[{time.monotonic():.4f}] Warning: Injected function returned {len(results_list)} results,"
                    + " but batch had {len(batch_info)} items. Cannot reliably match results."
                )
            ...
            for i in range(match_count):
                ...
                if req_id in self._pending_requests:
                    ...
                    print(
                        f"[{time.monotonic():.4f}] Warning: Received result for unknown"
                        + f" or already completed request ID {req_id[:6]}... Result: {result}"
                    )

        except Exception as e:
            submission_duration = time.monotonic() - start_time
            self._stats["total_batch_submission_duration_seconds"] += (
                submission_duration  # Still record time even on failure
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        if stats_copy["num_successful_batches"] > 0:
            stats_copy["avg_successful_batch_submission_duration_seconds"] = (
                ...
                if self._stats["num_successful_batches"] > 0
                else 0
            )  # Avoid division by zero if only failures
        else:
        ...
        if self._stats["actual_batch_sizes"]:
            stats_copy["avg_actual_batch_size"] = sum(self._stats["actual_batch_sizes"]) / len(
                self._stats["actual_batch_sizes"]
            )
        else:
