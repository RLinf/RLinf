...
def train_with_model(
    ...
    if resource_type == 0:
        ...
        load_spec_id = training_project.get("load_spec_id") or os.getenv(
            "INFINI_LOAD_SPEC_ID"
        )
    ...
    request_data = {
        ...
        "fault_tolerance": {
            ...
            "hang_check": {"enable": False, "timeout": 0},
            "environment_check": {
                "enable": True,
...
def _build_mount_config(training_project: dict) -> list:
    ...
    if volume_id:
        ...
        mount_path = (
            training_project.get("mount", {}).get("path")
            if isinstance(training_project.get("mount"), dict)
            else None
        )
        mount_rw_setting = (
            training_project.get("mount", {}).get("rw_setting")
            if isinstance(training_project.get("mount"), dict)
            else None
        )
        ...
        return [
            {
                ...
                "volume_id": volume_id,
                "rw_setting": mount_rw_setting
                or "can_write",  # 使用配置映射中的 rw_setting 或默认可写
            }
        ]
