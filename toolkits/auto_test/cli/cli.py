...
def main():
    ...
    push_parser.add_argument(
        "--api-key",
        type=str,
        help="API Key (默认: 从环境变量 INFINI_API_KEY 获取)"
    )
    push_parser.add_argument(
        "--base-url",
        type=str,
    ...
    # list 命令 - 列出所有可用的示例任务
    list_parser = subparsers.add_parser("list", help="列出所有可用的示例任务")
    ...
    run_parser = subparsers.add_parser("run", help="根据 config.py 启动任务（根据 task_type 自动判断是训练还是推理）")
    ...
    try:
        # 导入 API 客户端
        try:
            from submit_job.cli.api_client import make_api_request, get_api_key, get_base_url, get_account_id
        except ImportError:
            from api_client import make_api_request, get_api_key, get_base_url, get_account_id
        ...
        if args.command == "push":
            ...
            if args.project_dir:
                ...
                if example_dir.exists() and (example_dir / "config.py").exists():
                    ...
                    if not project_dir.exists():
                        ...
                        print(f"✗ 项目目录或示例任务 '{args.project_dir}' 不存在", file=sys.stderr)
                        if available:
            ...
            try:
                ...
                for i, volume in enumerate(volumes, 1):
                    ...
                    volume_name = volume.get("volume_name") or volume.get("name") or "N/A"
                    volume_type = volume.get("volume_type") or volume.get("type") or "N/A"
                ...
                print("使用方法:")
                print("  在 config.py 中设置 task_type，系统会自动从配置映射中获取 volume_id")
            ...
            job_api = (
                training_project.get("job_api")
                or config_globals.get("job_api")
            )
            ...
            if not account_id:
                print("✗ 未指定账户 ID。请使用 --account-id 参数或设置环境变量 INFINI_ACCOUNT_ID", file=sys.stderr)
                sys.exit(1)
            ...
            if not args.start_time and not args.end_time:
                ...
                start_time = int((now - timedelta(days=7)).timestamp() * 1_000_000)  # 向前推7天
                ...
                print(f"查询时间范围: {start_dt.strftime('%Y-%m-%d %H:%M:%S')} 至 {end_dt.strftime('%Y-%m-%d %H:%M:%S')}")
