...
def print_comparison_results(comparison_results: list):
    ...
    print("=" * 120)
    print(
        f"{'Experiment Name':<45} {'Baseline Name':<35} {'Similarity Method':<12} {'Similarity Value':<15} {'Common Steps':<10}"
    )
    print("=" * 120)
    ...
    for c in comparison_results:
        if "error" in c:
            print(
                f"{c['experiment_name']:<45} {'N/A':<35} {'N/A':<12} {'Error: ' + c['error']:<15}"
            )
            continue
        ...
        print(
            f"{c['experiment_name']:<45} {c['baseline_name']:<35} {method:<12} {sim_str:<15} {common_steps:<10}"
        )
...
def save_comparison_results(comparison_results: list, output_path: str | None = None):
    ...
    with open(output_path, "w") as f:
        f.write(
            "experiment_name,baseline_name,method,similarity,common_steps,baseline_log_path\n"
        )

        for c in comparison_results:
            if "error" in c:
                f.write(
                    f"{c['experiment_name']},N/A,N/A,Error: {c['error']},0,{c['baseline_log_path']}\n"
                )
                continue
            ...
            common_steps = sim_result.get("common_steps", 0)
            f.write(
                f"{c['experiment_name']},{c['baseline_name']},{method},{similarity},{common_steps},{c['baseline_log_path']}\n"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parse training logs, extract success_once metrics and check training status"
    )
    ...
    parser.add_argument("--no-plot", action="store_true", help="Do not plot curve")
    parser.add_argument(
        "--baseline",
        type=str,
        default=None,
    ...
    if args.baseline:
        ...
        if not args.no_plot:
            for i, result in enumerate(results):
                output_path = (
                    f"{result['experiment_name']}_vs_baseline.png"
                    if args.plot is None
                    else f"{args.plot.rsplit('.', 1)[0]}_{i}.png"
                    if len(results) > 1
                    else args.plot
                )
                try:
