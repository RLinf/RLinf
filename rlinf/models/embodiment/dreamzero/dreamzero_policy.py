...
class DreamZeroPolicy(nn.Module, BasePolicy):
    ...
    def unapply(self, batch, obs: dict = None, **kwargs):
        from tianshou.data import Batch  # noqa: F811
    ...
    def _load_pretrained_weights(self, pretrained_path: str) -> None:
        ...
        Parameters whose shape differs between the checkpoint and the current
        ...
        if skipped_shape:
            logger.info(
                "Shape-mismatched params (kept random init):\n%s",
                "\n".join(skipped_shape[:20]),
            )
