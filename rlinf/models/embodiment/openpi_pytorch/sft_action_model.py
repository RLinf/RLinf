...
class OpenPiPytorchSFTActionModel(OpenPiPytorchActionModel):
    ...
    def _observation_to_device(self, observation: Any) -> Observation:
        if not isinstance(observation, Observation):
            observation = Observation.from_dict(
                self._observation_to_vendored_dict(observation)
            )
        if not isinstance(observation, Observation):
            raise TypeError(
    ...
    @staticmethod
    def _observation_to_vendored_dict(observation: Any) -> dict[str, Any]:
        """Convert either vendored or upstream OpenPI observations to a dict."""
        if isinstance(observation, dict):
            observation = dict(observation)
        elif hasattr(observation, "to_dict"):
        ...
        if isinstance(observation, dict):
            if "images" in observation:
                observation = dict(observation)
                observation["image"] = observation.pop("images")
            if "image_masks" in observation:
                observation = dict(observation)
                observation["image_mask"] = observation.pop("image_masks")
            return observation
        if hasattr(observation, "images") and hasattr(observation, "image_masks"):
            return {
        ...
        raise TypeError(
            f"SFT observation must be an Observation-like object or dict; "
            f"got {type(observation)!r}."
        )
