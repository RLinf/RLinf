...
"""LeRobot v3 SO-101 SFT loader for the ``openpi_rlinf`` model path."""
...
class _RepackSO101(DataTransformFn):
    """Map LeRobot fields and scale degree-like joints before normalization."""
    ...
    def __call__(self, frame: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            "observation/image": np.asarray(frame["observation.images.wrist"]),
            "observation/state": np.asarray(
                frame["observation.state"], dtype=np.float32
            )
            * self.joint_scale,
            "actions": np.asarray(frame["action"], dtype=np.float32) * self.joint_scale,
            "prompt": prompt,
        }
...
def _read_dataset_metadata(root: pathlib.Path) -> int:
    ...
    for key in ("observation.state", "action"):
        ...
        if shape != (_RAW_ACTION_DIM,):
            raise ValueError(
                f"SO-101 {key} must have shape [{_RAW_ACTION_DIM}], got {shape}."
            )
    if "observation.images.wrist" not in features:
        raise ValueError("SO-101 dataset is missing observation.images.wrist.")
    return int(info["fps"])
...
def build_so101_sft_dataloader(
    ...
    if data_path is None:
        raise ValueError("openpi_rlinf SO-101 SFT requires data.train_data_paths.")
