...
class CollectiveGroup:
    ...
    def send_tensor_frame(
        ...
        """Send an ordered frame of pre-shaped tensor lanes.

        Unlike :meth:`send`, a tensor frame has no object-type, shape, dtype,
        or pickle metadata on the wire.  The caller must pair it with
        :meth:`recv_tensor_frame` using the same lane layout.
        """
        if not tensors:
            raise ValueError("Tensor frames must contain at least one tensor.")
        work = None
        for tensor in tensors:
            work = self.send_tensor(tensor, async_op=async_op, options=options)
        return work
    ...
    def recv_tensor_frame(
        ...
        """Receive an ordered frame into preallocated tensor lanes."""
        if not tensors:
            raise ValueError("Tensor frames must contain at least one tensor.")
        work = None
        for tensor in tensors:
            work = self.recv_tensor(tensor, async_op=async_op, options=options)
        return work
    ...
    def _get_object_info(self, object: torch.Tensor | Any) -> tuple[int, TensorData]:
        ...
        if isinstance(object, torch.Tensor):
            ...
            skeleton, tensors_list, metadata = extract_dataclass_tensor_fields(object)
            if tensors_list:
                (
                    cpu_tensor_mask,
                    cpu_tensors,
    ...
    def _send_tensor_dataclass(
        ...
        Args:
            ...
            comm_id (int): The ID for the send operation.
            tensor_data (TensorData): Pre-computed tensor skeleton and metadata.
            async_op (bool): Whether to perform the operation asynchronously.
    ...
    def _recv_tensor_dataclass(
        ...
        return unflatten_dataclass_tensor_fields(
            skeleton, metadata, flat_tensors
        ), pb_data
    ...
    def _broadcast_tensor_dataclass(
        ...
        """
        if self._rank == src_rank:
            skeleton, flat_tensors, metadata = extract_dataclass_tensor_fields(
                tensor_dataclass
            )
        else:
            metadata = None
        ...
        return unflatten_dataclass_tensor_fields(
            recv_skeleton, recv_metadata, recv_flat_tensors
        )
