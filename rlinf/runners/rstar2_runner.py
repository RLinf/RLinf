    ...
    def _run_agentloop_rollout(self):
        ...
        async def _async_rollout():
            try:
                ...
                # 准备采样参数
                sampling_params = {
                    "temperature": self.cfg.rollout.get("temperature", 0.7),
                    "max_new_tokens": self.cfg.rollout.get("max_new_tokens", 1024),
                    "top_p": self.cfg.rollout.get("top_p", 0.9),
                    "top_k": self.cfg.rollout.get("top_k", 50),
                }
                    ...
                    for group in request_groups:
                        ...
                        for raw_id, input_ids in enumerate(group.input_ids):
                            task = asyncio.create_task(
                                self._async_agentloop_generate(raw_id, input_ids, sampling_params)
                            )
        ...
        print(f"run 3")
        # self.rollout.offload_engine().wait()
    async def _async_agentloop_generate(self, raw_id: int, input_ids: List[int], sampling_params: Dict[str, Any]):
        """异步AgentLoop生成 - 直接调 ToolAgentLoop（token ids in/out）"""
        try:
            if self._tool_agent_loop is None:
                raise RuntimeError("ToolAgentLoop is not initialized")
            agent_output = await self._tool_agent_loop.run(input_ids, sampling_params)
            ...
            logging.error(f"Error in AgentLoop generate: {e}")
            raise
