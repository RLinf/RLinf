...
class FileSystemMCPSession:
    ...
    def __init__(self, session_id: str, config: DictConfig, mount_dir: Optional[str] = None):
        ...
        # Session resources
        self.client_session: Optional[ClientSession] = None  # not persisted across requests
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if self.mount_dir:
                ...
                self.temp_dir = tempfile.mkdtemp(prefix=f"mcp_filesystem_{self.session_id}_")
                self.logger.info(f"Created temp directory: {self.temp_dir}")
            
            # Get Docker image from config (store for per-request use)
            self.docker_image = self.config.get('mcp', {}).get('docker_image', 'mcp/filesystem:1.0.2')

            # Mark as ready: we will open/close stdio per request
            self.state = MCPSessionState.CONNECTED
            self.logger.info(f"MCP session {self.session_id} prepared successfully (per-request connection)")
            return True
    ...
    async def _create_server_params(self) -> StdioServerParameters:
        ...
        return StdioServerParameters(
            ...
            args=[
                "run", "--rm", "-i",
                "--mount", f"type=bind,src={self.temp_dir},dst=/projects/test",
                self.docker_image or 'mcp/filesystem:1.0.2', "/projects"
            ]
        )
            ...
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    if request.request_type == MCPRequestType.LIST_TOOLS:
                        ...
                        result_data = {
                            "resources": [{"uri": resource.uri, "name": resource.name} for resource in result.resources]
                        }
                        ...
                        result_data = {
                            "prompts": [{"name": prompt.name, "description": prompt.description} for prompt in result.prompts]
                        }
                        ...
                        for msg in result.messages:
                            ...
                            for content_item in msg.content:
                                if hasattr(content_item, 'text'):
                                    message_data["content"].append({"type": "text", "text": content_item.text})
                                else:
                                    message_data["content"].append({"type": str(type(content_item).__name__), "data": str(content_item)})
    ...
    async def cleanup(self):
        ...
        try:
            ...
            if self.temp_dir and not self.mount_dir and os.path.exists(self.temp_dir):
                ...
                self.logger.info(f"Preserved configured mount directory: {self.temp_dir}")
...
class MCPFilesystemClientWorker(Worker):
    ...
    async def _process_mcp_request(self, request: MCPRequest) -> MCPResponse:
        ...
        requested_mount_dir = request.metadata.get('mount_dir', getattr(self, 'mount_dir', None))
    ...
    async def _get_or_create_session(self, session_id: str, mount_dir: Optional[str]) -> Optional[FileSystemMCPSession]:
        ...
        async with asyncio.Lock():
            if session_id in self.sessions:
                ...
                if mount_dir and session.mount_dir and os.path.abspath(session.mount_dir) != os.path.abspath(mount_dir):
                    self.logger.error(
            ...
            if effective_mount_dir is None:
                try:
                    effective_mount_dir = tempfile.mkdtemp(prefix=f"mcp_session_{session_id}_")
                    self.log_info(f"Allocated temp mount_dir for session '{session_id}': {effective_mount_dir}")
                except Exception as e:
                    self.logger.error(f"Failed to allocate temp mount_dir for session '{session_id}': {e}")
                    return None
            if effective_mount_dir:
                try:
                    os.makedirs(effective_mount_dir, exist_ok=True)
                except Exception as e:
                    self.logger.error(f"Failed to ensure mount_dir '{effective_mount_dir}': {e}")
                    return None
            ...
            if await session.start():
                ...
                self.log_info(f"Created new MCP session: {session_id}, mount_dir: {effective_mount_dir}")
                return session
            else:
