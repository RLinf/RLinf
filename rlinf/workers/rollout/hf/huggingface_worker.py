...
class MutilStepRolloutWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        self.model_config, self.input_processor = get_model_config_and_processor(cfg.actor)
        ...
        for i in range(self._component_placement.rollout_world_size):
            self.channel.create_queue(f"{self._action_queue_name}_{i}", maxsize=cfg.rollout.channel.queue_size)
    ...
    def setup_sample_params(self):
        ...
        self._length_params = OmegaConf.to_container(self.cfg.algorithm.length_params, resolve=True)
        # sampling parameters for rollout
        self._sampling_params = OmegaConf.to_container(self.cfg.algorithm.sampling_params, resolve=True)
    ...
    def predict(self, processed_obs, do_sample=True, mode="train"):
        ...
        sample_kwargs = self._train_sampling_params if mode == "train" else self._eval_sampling_params
        ...
        chunk_logprobs = default_logits_processor(
            action_logits, action_tokens, self.hf_model.vocab_size, self.hf_model.config.n_action_bins
        )["logprobs"]

        chunk_values = None
        if self.cfg.algorithm.require_values:
            if self.cfg.actor.model.vh_mode == "a0":
                hidden_features = last_hidden_state[:, -action_token_len]  # [batch_size, hidden_dim]
                chunk_values = self.hf_model.value_head(hidden_features)  # [batch_size, 1]

        if chunk_values is None:
            chunk_values = torch.zeros_like(chunk_logprobs[..., :1])


        chunk_actions = actions.reshape(-1, self.hf_model.num_action_chunks,  self.hf_model.action_dim)
        chunk_action_tokens = action_tokens.reshape(-1, self.hf_model.num_action_chunks,  self.hf_model.action_dim)
    ...
    def update_env_batch(self, i, env_batch):
        ...
        self.buffer_list[i]["dones"].append(env_batch["dones"].bool().cpu().contiguous())
        ...
        if env_batch["dones"].any() and self.cfg.env.train.auto_reset:
            if self.cfg.algorithm.require_values:
                ...
                self.buffer_list[i]["rewards"][-1][:, -1] += self.cfg.algorithm.gamma * final_values.cpu()
    ...
    async def generate(self):
        ...
        for rollout_epoch in range(self.cfg.algorithm.rollout_epoch):
            ...
            for step in tqdm(range(self.cfg.algorithm.n_chunk_steps), desc=f"Rollout Epoch {rollout_epoch} in Generate Step"):
                ...
                for i in range(self.stage_num):
                    ...
                    chunk_actions, chunk_action_token, chunk_logprobs, chunk_values = self.predict(processed_obs)
                    await self.send_chunk_actions(chunk_actions)

                    self.buffer_list[i]["input_ids"].append(processed_obs["input_ids"].cpu().contiguous())
                    self.buffer_list[i]["pixel_values"].append(processed_obs["pixel_values"].cpu().contiguous())
                    self.buffer_list[i]["attention_mask"].append(processed_obs['attention_mask'].bool().cpu().contiguous())
                    self.buffer_list[i]["action_tokens"].append(chunk_action_token.cpu().contiguous())
                    self.buffer_list[i]["prev_logprobs"].append(chunk_logprobs.cpu().contiguous())
                    self.buffer_list[i]["prev_values"].append(chunk_values.cpu().contiguous())
    ...
    async def evaluate(self):
        ...
        for step in tqdm(range(self.cfg.algorithm.n_eval_chunk_steps), desc="Rollout in Eval Step"):
            for i in range(self.stage_num):
                env_batch = await self.recv_env_batch()
                processed_obs = prepare_observations(
    ...
    async def recv_env_batch(self):
        env_batch = await self.channel.get(queue_name=f"{self._obs_queue_name}_{self._rank}", async_op=True).async_wait()
        return env_batch

    async def send_chunk_actions(self, chunk_actions):
        await self.channel.put(item=chunk_actions, queue_name=f"{self._action_queue_name}_{self._rank}", async_op=True).async_wait()
    ...
    async def send_rollout_batch(self, stage_id):
        ...
        for i in range(split_num):
            ...
            for key in rollout_batch.keys():
                if "env_info/" not in key:
                    rollout_batch_i[key] = torch.chunk(rollout_batch[key], split_num, dim=1)[i].contiguous()
                else:
                    rollout_batch_i[key] = torch.chunk(rollout_batch[key], split_num, dim=0)[i].contiguous()
            await self.channel.put(item=rollout_batch_i, queue_name=self._replay_buffer_name, async_op=True).async_wait()
