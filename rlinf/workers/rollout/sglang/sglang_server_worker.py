...
def _build_server_args_from_config(
    ...
    *,
    ...
    supported_args = set(inspect.signature(ServerArgs).parameters)
    server_kwargs = {
        key: value for key, value in server_kwargs.items() if key in supported_args
    }
...
class SGLangServerWorker(Worker):
    def __init__(
        ...
        cfg_rollout = (
            ...
            else getattr(config, "rollout", None)
        )
        self._server_proc: Optional[mp.Process] = None
        self._return_logprobs = cfg_rollout.return_logprobs
        self._openai_response_tokens = bool(
            config.get("agentlightning", {}).get("openai_response_tokens", False)
        )
    ...
    def server_start(self):
        ...
        def _run():
            ...
            launch_server(
                server_args,
                rollout_return_logprobs=self._return_logprobs,
                openai_response_tokens=self._openai_response_tokens,
            )
    ...
    def _placement_to_minimal_dict(
        ...
        return {
            ...
            "_rollout_sync_mode": placement._rollout_sync_mode.value
            if hasattr(placement._rollout_sync_mode, "value")
            else int(placement._rollout_sync_mode),
        }
    ...
    def offload_engine(self):
        assert self.weight_reload is not None, (
            "offload_engine requires weight_reload is not None"
        )
    ...
    def onload_engine(self):
        assert self.weight_reload == "cpu", (
            "onload_engine requires weight_reload == 'cpu'"
        )
