...
class EnvWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        assert self._component_placement.rollout_world_size % self._component_placement.env_world_size == 0
        # gather_num: number of rollout for each env process
        self.gather_num = self._component_placement.rollout_world_size // self._component_placement.env_world_size
        # stage_num: default to 2, use for pipeline rollout process
        self.stage_num = self.cfg.rollout.pipeline_stage_num
        self.batch_size = self.cfg.env.train.num_group * self.cfg.env.train.group_size
        self.eval_batch_size = self.cfg.env.eval.num_group * self.cfg.env.eval.group_size
        ...
        for i in range(self.gather_num):
            self.channel.create_queue(f"{self._obs_queue_name}_{i + self._rank * self.gather_num}", maxsize=cfg.env.channel.queue_size)
        ...
        if self.cfg.env.train.simulator_type == "maniskill":
            ...
            if not only_eval:
                for _ in range(self.stage_num):
                    self.simulator_list.append(EnvManager(self.cfg.env.train, rank=self._local_rank, env_cls=ManiskillEnv, enable_offload=enable_offload))
            if self.cfg.runner.val_check_interval > 0 or only_eval:
                for _ in range(self.stage_num):
                    self.eval_simulator_list.append(EnvManager(self.cfg.env.eval, rank=self._local_rank, env_cls=ManiskillEnv, enable_offload=enable_offload))
        elif self.cfg.env.train.simulator_type == "libero":
            from rlinf.envs.libero_env import LiberoEnv
            if not only_eval:
                for _ in range(self.stage_num):
                    self.simulator_list.append(EnvManager(self.cfg.env.train, rank=self._local_rank, env_cls=LiberoEnv, enable_offload=enable_offload))
            if self.cfg.runner.val_check_interval > 0 or only_eval:
                for _ in range(self.stage_num):
                    self.eval_simulator_list.append(EnvManager(self.cfg.env.eval, rank=self._local_rank, env_cls=LiberoEnv, enable_offload=enable_offload))
    ...
    async def recv_chunk_actions(self):
        ...
        for gather_id in range(self.gather_num):
            chunk_action.append(await self.channel.get(queue_name=f"{self._action_queue_name}_{gather_id + self._rank * self.gather_num}", async_op=True).async_wait())
        ...
        for key, value in env_batch.items():
            if isinstance(value, torch.Tensor):
                ...
                if mode == "train":
                    ...
                    assert length == self.eval_batch_size, f"key {key}, length: {length}, batch_size: {self.eval_batch_size}"
                env_batch_i[key] = value[gather_id * length // self.gather_num: (gather_id + 1) * length // self.gather_num]
    ...
    async def send_env_batch(self, env_batch, mode="train"):
        ...
        for gather_id in range(self.gather_num):
            ...
            await self.channel.put(item=env_batch_i, queue_name=f"{self._obs_queue_name}_{gather_id + self._rank * self.gather_num}", async_op=True).async_wait()
        ...
        for rollout_epoch in range(self.cfg.algorithm.rollout_epoch):
            ...
            if not self.cfg.env.train.auto_reset:
                for i in range(self.stage_num):
                    ...
                    extracted_obs, rewards, terminations, truncations, infos = self.simulator_list[i].step()
                    dones = torch.logical_or(terminations, truncations).unsqueeze(1).repeat(1, self.cfg.actor.model.num_action_chunks)
                ...
                infos = {}
                for i in range(self.stage_num):
                    env_batch = create_env_batch(self.last_obs_list[i], None, self.last_dones_list[i], infos)
                    env_batch_list.append(env_batch)
    ...
    async def evaluate(self):
        for i in range(self.stage_num):
            ...
            extracted_obs, rewards, terminations, truncations, infos = self.eval_simulator_list[i].step()
