...
def compute_rollout_metrics_dynamic(
    ...
    tool_call_metrics = {}
    if (
        "turn_subtask_counts" in rollout_batch
        and rollout_batch["turn_subtask_counts"] is not None
    ):
        ...
        local_sum_traj_search_plus_access = (
            (traj_search_counts + traj_access_counts).sum().item()
        )
        ...
        local_max_traj_search_plus_access = (
            (traj_search_counts + traj_access_counts).max().item()
        )
        ...
        local_sum_turn_search_plus_access = (
            (turn_search_counts + turn_access_counts).sum().item()
        )
        ...
        local_max_turn_search_plus_access = (
            (turn_search_counts + turn_access_counts).max().item()
        )

        # All-reduce trajectory-level sums
        traj_sum_metrics = torch.as_tensor(
            [
                local_sum_traj_subtask,
                local_sum_traj_search,
                local_sum_traj_access,
                local_sum_traj_search_plus_access,
            ],
            device=device,
            dtype=torch.float32,
        ...
        torch.distributed.all_reduce(
            traj_sum_metrics, torch.distributed.ReduceOp.SUM, group=data_parallel_group
        )
        (
        ...
        ) = traj_sum_metrics.tolist()

        # All-reduce trajectory-level maxs
        traj_max_metrics = torch.as_tensor(
            [
                local_max_traj_subtask,
                local_max_traj_search,
                local_max_traj_access,
                local_max_traj_search_plus_access,
            ],
            device=device,
            dtype=torch.float32,
        ...
        torch.distributed.all_reduce(
            traj_max_metrics, torch.distributed.ReduceOp.MAX, group=data_parallel_group
        )
        (
        ...
        ) = traj_max_metrics.tolist()

        # All-reduce turn-level sums
        turn_sum_metrics = torch.as_tensor(
            [
                local_sum_turn_subtask,
                local_sum_turn_search,
                local_sum_turn_access,
                local_sum_turn_search_plus_access,
            ],
            device=device,
            dtype=torch.float32,
        ...
        torch.distributed.all_reduce(
            turn_sum_metrics, torch.distributed.ReduceOp.SUM, group=data_parallel_group
        )
        (
        ...
        ) = turn_sum_metrics.tolist()

        # All-reduce turn-level maxs
        turn_max_metrics = torch.as_tensor(
            [
                local_max_turn_subtask,
                local_max_turn_search,
                local_max_turn_access,
                local_max_turn_search_plus_access,
            ],
            device=device,
            dtype=torch.float32,
        ...
        torch.distributed.all_reduce(
            turn_max_metrics, torch.distributed.ReduceOp.MAX, group=data_parallel_group
        )
        (
        ...
        torch.distributed.all_reduce(
            valid_turn_counts, torch.distributed.ReduceOp.SUM, group=data_parallel_group
        )
        global_num_valid_planner_turns, global_num_valid_worker_turns = (
            valid_turn_counts.tolist()
        )

        # Compute final metrics
        # Trajectory-level means: divide by total number of trajectories across all DPs
        tool_call_metrics["trajectory/mean/subtask"] = (
            global_sum_traj_subtask / num_trajectories
        )
        tool_call_metrics["trajectory/mean/search"] = (
            global_sum_traj_search / num_trajectories
        )
        tool_call_metrics["trajectory/mean/access"] = (
            global_sum_traj_access / num_trajectories
        )
        tool_call_metrics["trajectory/mean/search+access"] = (
            global_sum_traj_search_plus_access / num_trajectories
        )
        ...
        tool_call_metrics["trajectory/max/search+access"] = int(
            global_max_traj_search_plus_access
        )
