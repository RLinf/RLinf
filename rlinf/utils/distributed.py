...
def compute_rollout_metrics_dynamic(
    ...
    traj_subtask_counts = torch.zeros(num_trajectories, dtype=torch.long, device=device)
    traj_search_counts = torch.zeros(num_trajectories, dtype=torch.long, device=device)
    traj_access_counts = torch.zeros(num_trajectories, dtype=torch.long, device=device)    
    ...
    # Compute tool call metrics if available
    tool_call_metrics = {}
    if "turn_subtask_counts" in rollout_batch and rollout_batch["turn_subtask_counts"] is not None:
        ...
        # Aggregate turn counts to trajectory level using idx_to_traj
        ...
        # Compute local sums for trajectory-level metrics
        local_sum_traj_subtask = traj_subtask_counts.sum().item()
        local_sum_traj_search = traj_search_counts.sum().item()
        local_sum_traj_access = traj_access_counts.sum().item()
        local_sum_traj_search_plus_access = (traj_search_counts + traj_access_counts).sum().item()

        local_max_traj_subtask = traj_subtask_counts.max().item()
        local_max_traj_search = traj_search_counts.max().item()
        local_max_traj_access = traj_access_counts.max().item()
        local_max_traj_search_plus_access = (traj_search_counts + traj_access_counts).max().item()

        # Compute local sums and maxs for turn-level metrics (only over valid turns)
        # For planner: only count turns with subtask calls
        # For worker: only count turns with search or access calls
        local_sum_turn_subtask = turn_subtask_counts.sum().item()
        local_sum_turn_search = turn_search_counts.sum().item()
        local_sum_turn_access = turn_access_counts.sum().item()
        local_sum_turn_search_plus_access = (turn_search_counts + turn_access_counts).sum().item()

        local_max_turn_subtask = turn_subtask_counts.max().item()
        local_max_turn_search = turn_search_counts.max().item()
        local_max_turn_access = turn_access_counts.max().item()
        local_max_turn_search_plus_access = (turn_search_counts + turn_access_counts).max().item()

        # All-reduce trajectory-level sums
        traj_sum_metrics = torch.as_tensor(
            [local_sum_traj_subtask, local_sum_traj_search, local_sum_traj_access, local_sum_traj_search_plus_access],
            device=device, dtype=torch.float32
        )
        torch.distributed.all_reduce(traj_sum_metrics, torch.distributed.ReduceOp.SUM, group=data_parallel_group)
        global_sum_traj_subtask, global_sum_traj_search, global_sum_traj_access, global_sum_traj_search_plus_access = traj_sum_metrics.tolist()

        # All-reduce trajectory-level maxs
        traj_max_metrics = torch.as_tensor(
            [local_max_traj_subtask, local_max_traj_search, local_max_traj_access, local_max_traj_search_plus_access],
            device=device, dtype=torch.float32
        )
        torch.distributed.all_reduce(traj_max_metrics, torch.distributed.ReduceOp.MAX, group=data_parallel_group)
        global_max_traj_subtask, global_max_traj_search, global_max_traj_access, global_max_traj_search_plus_access = traj_max_metrics.tolist()

        # All-reduce turn-level sums
        turn_sum_metrics = torch.as_tensor(
            [local_sum_turn_subtask, local_sum_turn_search, local_sum_turn_access, local_sum_turn_search_plus_access],
            device=device, dtype=torch.float32
        )
        torch.distributed.all_reduce(turn_sum_metrics, torch.distributed.ReduceOp.SUM, group=data_parallel_group)
        global_sum_turn_subtask, global_sum_turn_search, global_sum_turn_access, global_sum_turn_search_plus_access = turn_sum_metrics.tolist()

        # All-reduce turn-level maxs
        turn_max_metrics = torch.as_tensor(
            [local_max_turn_subtask, local_max_turn_search, local_max_turn_access, local_max_turn_search_plus_access],
            device=device, dtype=torch.float32
        )
        torch.distributed.all_reduce(turn_max_metrics, torch.distributed.ReduceOp.MAX, group=data_parallel_group)
        global_max_turn_subtask, global_max_turn_search, global_max_turn_access, global_max_turn_search_plus_access = turn_max_metrics.tolist()

        # All-reduce valid turn counts
        valid_turn_counts = torch.as_tensor(
            [local_num_valid_planner_turns, local_num_valid_worker_turns],
            device=device, dtype=torch.float32
        )
        torch.distributed.all_reduce(valid_turn_counts, torch.distributed.ReduceOp.SUM, group=data_parallel_group)
        global_num_valid_planner_turns, global_num_valid_worker_turns = valid_turn_counts.tolist()

        # Compute final metrics
        # Trajectory-level means: divide by total number of trajectories across all DPs
        tool_call_metrics["trajectory/mean/subtask"] = global_sum_traj_subtask / num_trajectories
        tool_call_metrics["trajectory/mean/search"] = global_sum_traj_search / num_trajectories
        tool_call_metrics["trajectory/mean/access"] = global_sum_traj_access / num_trajectories
        tool_call_metrics["trajectory/mean/search+access"] = global_sum_traj_search_plus_access / num_trajectories

        # Trajectory-level maxs
        tool_call_metrics["trajectory/max/subtask"] = int(global_max_traj_subtask)
        tool_call_metrics["trajectory/max/search"] = int(global_max_traj_search)
        tool_call_metrics["trajectory/max/access"] = int(global_max_traj_access)
        tool_call_metrics["trajectory/max/search+access"] = int(global_max_traj_search_plus_access)

        # Turn-level means: divide by number of valid turns
        # Only compute planner metrics if there are valid planner turns
        ...
        # Only compute worker metrics if there are valid worker turns
        ...
        # Clean up tool call metrics fields from rollout_batch after computing metrics
        rollout_batch.pop("turn_subtask_counts", None)
        rollout_batch.pop("turn_search_counts", None)
        rollout_batch.pop("turn_access_counts", None)
        rollout_batch.pop("num_valid_planner_turns", None)
        rollout_batch.pop("num_valid_worker_turns", None)
    ...
    rollout_metrics = {
        ...
        **tool_call_metrics,  # Add tool call metrics
    }
