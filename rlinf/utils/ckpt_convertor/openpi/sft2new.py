...
"""Convert an RLinf SFT checkpoint to the new OpenPI PyTorch layout.
...
def resolve_model_spec(config_name: str) -> Sft2NewModelSpec:
...
def _validate_state_dict(
    state_dict: Mapping[str, torch.Tensor], spec: Sft2NewModelSpec
) -> None:
...
def _validate_against_reference(
    ...
    spec: Sft2NewModelSpec,
) -> None:
    """Validate keys and tensor shapes against a matching new-format model."""
    ...
    if missing or unexpected:
        raise ValueError(
            "SFT checkpoint keys do not match the reference model for "
...
def add_arguments(parser) -> None:
    ...
    parser.add_argument(
        "--output-model",
        required=True,
        help="output new-format checkpoint dir with config.json + model.safetensors",
    )
    ...
    parser.add_argument(
        "--reference-model",
        default=None,
        help="optional matching new-format model used to validate keys and shapes",
    )
