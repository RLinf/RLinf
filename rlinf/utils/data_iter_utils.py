...
def get_iterator_dynamic(
    ...
    if isinstance(batch, (dict, UserDict)):
        ...
        while valid_max_token == 0:
            ...
            for partition in partitions:
                ...
                if sum(curr_batch["prompt_lengths"]) + sum(curr_batch["response_lengths"]) > max_tokens_per_mbs:
                    valid_max_token = 0
                    print(
    ...
    for m_batch in microbatches:
        if sum(m_batch["prompt_lengths"]) + sum(m_batch["response_lengths"]) > max_tokens_per_mbs:
            breakpoint()
