...
class MaModelParallelComponentPlacement(ComponentPlacement):
    """Component placement for model-parallel components.
    ...
    def __init__(self, config: DictConfig, cluster: Cluster):
        ...
        self.rollout_names = self._config.cluster.rollout_names
        assert isinstance(self.rollout_names, omegaconf.listconfig.ListConfig), (
            f"bad type: {type(self.rollout_names)}"
        )
        assert len(self.rollout_names) > 1
        self._list_rollout_gpus = [
            self._get_component_hardware(rollout_name)
            for rollout_name in self.rollout_names
        ]
        assert self._list_rollout_gpus is not None, (
            "Rollout GPUs must be specified in the component_placement config."
        )
        assert len(self._list_rollout_gpus) >= 1
        self._rollout_gpus = self._list_rollout_gpus[0]
        self._all_rollout_gpus = sorted([j for i in self._list_rollout_gpus for j in i])
        ...
        assert self._all_rollout_gpus == list(
            range(self._all_rollout_gpus[0], self._all_rollout_gpus[-1] + 1)
        ), f"Rollout GPUs {self._all_rollout_gpus} must be continuous."
        if self._inference_gpus is not None:
            assert self._inference_gpus == list(
                range(self._inference_gpus[0], self._inference_gpus[-1] + 1)
            ), f"Inference GPUs {self._inference_gpus} must be continuous."
    ...
    def _is_auto(self):
        ...
        assert (
            self._actor_gpus[0] == 0
            and self._actor_gpus[-1] == self._all_rollout_gpus[0] - 1
        ), order_error_msg
        if self._inference_gpus is None:
            assert self._all_rollout_gpus[-1] == self._cluster_num_gpus - 1, (
                order_error_msg
            )
        else:
            assert self._all_rollout_gpus[-1] == self._inference_gpus[0] - 1, (
                order_error_msg
            )
            assert self._inference_gpus[-1] == self._cluster_num_gpus - 1, (
                order_error_msg
    ...
    def _is_collocated(self):
        if self._actor_gpus == self._all_rollout_gpus:
            return True
        return False

    def _is_disaggregated(self):
        actor_gpu_set = set(self._actor_gpus)
        rollout_gpu_set = set(self._all_rollout_gpus)
        inference_gpu_set = (
            [] if self._inference_gpus is None else set(self._inference_gpus)
        )
        return (
    ...
    def _generate_placements(self):
        if self._placement_mode == PlacementMode.COLLOCATED:
            ...
            self._placements["rollout"] = []
            for idx, rollout_gpus in enumerate(self._list_rollout_gpus):
                self._placements["rollout"].append(
                    PackedPlacementStrategy(
                        rollout_gpus[0],
                        rollout_gpus[-1],
                        num_hardware_per_process=self.rollout_tp_size,
    ...
    def _get_component_hardware(self, component_name: str):
        if component_name not in self._component_rank_map:
            return None
        return self.get_hardware_ranks(component_name)
