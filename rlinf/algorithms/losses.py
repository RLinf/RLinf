...
@register_policy_loss("embodied_sft")
def compute_embodied_sft_loss(
    logprobs: torch.Tensor,
    old_logprobs: torch.Tensor,
    advantages: torch.Tensor,
    loss_mask: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    """Negative log-likelihood loss for embodied SFT (behavior cloning).

    The model's ``default_forward`` evaluates the log-probability of
    demonstration actions under the predicted Gaussian distribution.
    Minimising ``-logprobs`` maximises the likelihood of the
    demonstration data — the standard imitation-learning objective.

    *advantages* and *old_logprobs* are accepted for compatibility with
    the existing training loop but are **ignored** (BC does not use
    advantage-weighted updates).

    Args:
        logprobs: Log-probability of each demo action.  Shape
            ``[T*B, action_dim]`` (token-level) or ``[T*B]``
            (action-level), depending on ``logprob_type``.
        old_logprobs: Ignored (kept for API compatibility).
        advantages: Ignored (kept for API compatibility).
        loss_mask: Boolean mask of valid time-steps, same shape as
            *logprobs*.
        loss_agg_func: Aggregation function (default: mean over valid
            elements).
    ...
    nll = -logprobs * loss_mask
    token_num = loss_mask.sum().clamp(min=1)
    loss = nll.sum() / token_num

    with torch.no_grad():
        # MSE between predicted mean and demo action, for diagnostics.
        # We can't compute MSE here without the predicted mean, so just
        # track the NLL.
        approx_kl = torch.zeros(1, device=logprobs.device)
        clip_fraction = torch.zeros(1, device=logprobs.device)

    metrics_data = {
        "actor/policy_loss": loss.detach(),
        "actor/token_num": token_num.detach(),
        "actor/policy_loss_mbs_mean": loss.detach(),
        "actor/policy_loss_abs": loss.detach(),
        "actor/ratio": torch.ones(1, device=logprobs.device),
        "actor/clipped_ratio": torch.zeros(1, device=logprobs.device),
        "actor/dual_cliped_ratio": torch.zeros(1, device=logprobs.device),
