...
def pack_sequences(
    input_tensor: torch.Tensor,  # [B, unpacked_seq_len]
    ...
    Args:
        input_tensor: [B, unpacked_seq_len], may contain padding.
        idx_starts, idx_ends: Valid range per sample (length B); idx_ends is exclusive.
        max_seq_len: Target packed length. If None, no padding will be performed.
        pad_val: Fill value for padding.

    Returns:
        1D tensor of shape (packed_seq_len). Use unsqueeze(0) for [1, packed_seq_len].
    """
    ...
    for idx in range(input_tensor.shape[0]):
        input_tensors.append(input_tensor[idx, idx_starts[idx] : idx_ends[idx]])

    if pad_len > 0:
        pad_tensor = torch.full(
            (pad_len,), pad_val, dtype=input_tensor.dtype, device=input_tensor.device
        )
        input_tensors.append(pad_tensor)

    # [1, packed_seq_len]
    output_tensor = torch.cat(input_tensors)
    return output_tensor


def unpack_sequences(
    input_tensor: torch.Tensor,  # [1, packed_seq_len]
    idx_starts: list[int],  # [B]
    idx_ends: list[int],  # [B]
    max_seq_len: int,
    pad_val: int,
    ...
    Args:
        input_tensor: Packed sequence [1, packed_seq_len].
        idx_starts, idx_ends: Start/end (exclusive) in packed sequence per sample (length B).
        max_seq_len: Sequence length per sample after unpacking.
        pad_val: Fill for non-valid positions.

    Returns:
        Tensor [B, unpacked_seq_len]; valid content in [idx_start, idx_end), rest pad_val.
    """
    assert len(input_tensor.shape) == 2
    ...
    output_tensor = torch.stack(
        [
            torch.nn.functional.pad(
                input_split, (idx_start, max_seq_len - idx_end), "constant", pad_val
            )
...
def pack_fsdp_input(
    ...
    *,
    ...
    """Pack FSDP training input_ids and position_ids from [B, seq_len] to [1, packed_seq_len].
    ...
    Args:
        ...
        max_seq_len: Packed length. eos_token_id: Pad value for input_ids end. If None, no padding will be performed.

    Returns:
        input_ids, position_ids: [1, packed_seq_len]. attention_mask: None.
    """
    input_ids = pack_sequences(
        input_ids, idx_starts, idx_ends, max_seq_len, eos_token_id
    ).unsqueeze(0)
    position_ids = pack_sequences(
        position_ids, idx_starts, idx_ends, max_seq_len, 0
    ).unsqueeze(0)
...
def unpack_fsdp_logprobs(
    ...
    *,
    ...
    """Compute logprobs from packed model output and unpack to per-sample [B, max_seq_len].
    ...
    Args:
        ...
        idx_starts, idx_ends: From prepare_pack_fsdp. max_seq_len: Seq length per sample after unpack.
        eos_token_id: Appended when building targets. compute_logprobs_fn: (logits, response_ids) -> logprobs.

    Returns:
        Logprobs [B, max_seq_len]; valid positions = log prob, rest = 0.
    """
    responses = torch.nn.functional.pad(
        input_ids[:, 1:], (0, 1), "constant", eos_token_id
    )
    logprobs = compute_logprobs_fn(logits, responses)
    logprobs = torch.nn.functional.pad(logprobs[:, :-1], (1, 0), "constant", 0)
    logprobs = unpack_sequences(logprobs, idx_starts, idx_ends, max_seq_len, pad_val=0)
    return logprobs
