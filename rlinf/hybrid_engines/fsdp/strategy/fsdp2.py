                    ...
                    if buffer_name == "inv_freq" and "rotary" in module_name.lower():
                        try:
                            ...
                            if rope_init_fn is not None and module_config is not None:
                                ...
                                rope_parameters = hf_rope_parameters(module_config)
                                rope_type = rope_parameters.get("rope_type", "default")
                                if rope_type != "default":
                                    raise RuntimeError(
                                        "Cannot materialize scaled RoPE buffer without "
                                        "Transformers rope_init_fn. Please use a compatible "
                                        "Transformers version."
                                    )
                                inv_freq = _build_rope_inv_freq(module, module_config)
                            ...
                            if rope_type != "default":
                                raise RuntimeError(
                                    "Failed to materialize scaled RoPE buffer with "
                                    "Transformers rope_init_fn. Please use a compatible "
                                    "Transformers version."
                                ) from error
                            inv_freq = _build_rope_inv_freq(module, module_config)
