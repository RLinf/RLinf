...
class FSDPModelManager:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        assert self.torch_dtype == torch.float16 or self.torch_dtype == torch.bfloat16, f"Precision {self._cfg.model.precision} is not supported, only support bf16 and fp16."
    ...
    def model_provider_func(self) -> torch.nn.Module:
        if self._cfg.model.get("gptq_model", False):
            ...
            model = AutoModelForCausalLM.from_pretrained(
                self._cfg.model.model_path, device_map=self._cfg.model.get("device_map", "auto"), load_in_8bit=True
            )
        else:
    ...
    def setup_model_and_optimizer(self):
        ...
        mixed_precision = MixedPrecision(param_dtype=self.torch_dtype, reduce_dtype=self.torch_dtype, buffer_dtype=self.torch_dtype)
        ...
        param_groups = [
            {"params": [p for n, p in self.model.named_parameters() if "value_head" not in n and p.requires_grad], "lr": self._cfg.optim.lr, 'betas': betas},
        ]

        if self._cfg.model.vh_mode in ['a', 'a0', 'a6']:
            param_groups.append({"params": [p for n, p in self.model.named_parameters() if "value_head" in n and p.requires_grad], "lr": self._cfg.optim.value_lr, 'betas': betas})
    ...
    def offload_fsdp_param_and_grad(self, offload_grad=False):
        for _, param in self.model.named_parameters():
            if hasattr(param, '_handle') and param._handle is not None:
                ...
                if hasattr(flat_param, '_local_shard') and flat_param._local_shard is not None:
                    flat_param._local_shard = flat_param._local_shard.to("cpu", non_blocking=True)
    ...
    def load_fsdp_param_and_grad(self, device_id, load_grad=False):
        for _, param in self.model.named_parameters():
            if hasattr(param, '_handle') and param._handle is not None:
                flat_param = param._handle.flat_param
                if hasattr(flat_param, '_local_shard') and flat_param._local_shard is not None:
                    flat_param._local_shard = flat_param._local_shard.to(device_id, non_blocking=True)
                if flat_param.data is not None:
