...
class BaseSimManager(ABC):
    ...
    def _wait_ready(self) -> None:
        ...
        logger.info(
            "Waiting for simulation readiness (timeout=%ds)...",
        ...
        while time.time() < deadline:
            ...
            try:
                ...
                for line in log_tail.splitlines():
                    ...
                    if not stripped or stripped in _shown_log_lines:
                        continue
                    if any(
                        pat.lower() in stripped.lower()
                        for pat in self._ISAAC_LOG_PATTERNS
                    ):
            ...
            if now - last_heartbeat >= _HEARTBEAT_SEC:
                ...
                if last_stage in ("", "mujoco_launching"):
                    ...
                    hint = (
                        "Isaac Sim GPU init + USD scene load (this is the long part)..."
                    )
                else:
                    hint = "finalising..."
                logger.info(
                    "  › %s  elapsed=%.0fs  remaining≤%.0fs", hint, elapsed, remaining
                )
                last_heartbeat = now
