...
class Turtle2Env(gym.Env):
    ...
    def __init__(
        ...
        self.config = config
        self._normalize_config_arrays()
        if self.config.action_mode not in {"relative_pose", "absolute_pose"}:
            raise ValueError(
    ...
    def _normalize_config_arrays(self) -> None:
        """Normalize array-like config values after YAML/dataclass construction."""
        for name in (
            "target_ee_pose",
            "reset_ee_pose",
            "ee_pose_limit_min",
    ...
    def _init_action_obs_spaces(self):
        ...
        self._relative_pose_action_space = gym.spaces.Box(
            np.ones(
                (len(self.config.use_arm_ids) * ACTION_DIM_PER_ARM), dtype=np.float32
            )
            * -1,
            np.ones(
                (len(self.config.use_arm_ids) * ACTION_DIM_PER_ARM), dtype=np.float32
        ...
        self.action_space = (
            self._absolute_pose_action_space
            if self.config.action_mode == "absolute_pose"
            else self._relative_pose_action_space
        )
    ...
    def step_relative_pose(
        ...
        action = np.clip(
            action,
            self._relative_pose_action_space.low,
            self._relative_pose_action_space.high,
        )
    ...
    def step_absolute_pose(
        ...
        Args:
            action: Absolute end-effector action of shape ``(7,)`` for single arm
                or ``(14,)`` for dual arm (xyz, rpy, gripper per arm).
