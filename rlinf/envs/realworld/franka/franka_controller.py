    ...
    def __init__(
        ...
        for p in ros_python_paths:
            if os.path.isdir(p) and p not in sys.path:
                sys.path.insert(0, p)
                self._logger.info("Patched sys.path to include %s", p)

        # Also patch PYTHONPATH env var so that subprocesses (roscore,
        # roslaunch, etc.) can find ROS modules even when they are spawned
        # under the venv Python via `#!/usr/bin/env python3`.
        pythonpath = os.environ.get("PYTHONPATH", "")
        pythonpath_parts = pythonpath.split(":") if pythonpath else []
        for p in ros_python_paths:
            if os.path.isdir(p) and p not in pythonpath_parts:
                pythonpath_parts.insert(0, p)
        os.environ["PYTHONPATH"] = ":".join(pythonpath_parts)
        self._logger.info("PYTHONPATH=%s", os.environ["PYTHONPATH"])
