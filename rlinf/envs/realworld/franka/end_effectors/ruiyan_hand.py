...
The low-level protocol implementation is embedded here so that no
external ``ruiyan_hand_driver`` package is required at runtime.
"""
...
_proto_logger = logging.getLogger(__name__)
...
class _SerialLink:
    ...
    def connect(self) -> None:
        ...
        _proto_logger.info("Ruiyan serial opened: %s", self._port)
...
class RuiyanHand(EndEffector):
    ...
    def __init__(
        ...
        self._target_positions = np.zeros(self._NUM_DOFS, dtype=np.float64)
        self._current_positions = np.zeros(self._NUM_DOFS, dtype=np.float64)
        self._current_velocities = np.zeros(self._NUM_DOFS, dtype=np.float64)
        self._current_currents = np.zeros(self._NUM_DOFS, dtype=np.float64)
        self._current_statuses = np.zeros(self._NUM_DOFS, dtype=np.int32)
    ...
    def get_detailed_state(self) -> dict:
        ...
        Returns:
            ...
            - ``finger_names`` – list of 6 DOF names.
            - ``positions`` – normalised positions ``[0, 1]``.
            - ``velocities`` – raw motor velocities (signed 12-bit).
            - ``currents`` – raw motor currents (signed 12-bit).
            - ``statuses`` – per-motor status bytes (0 = normal).
            - ``motor_ids`` – physical motor IDs.
            - ``port`` – serial device path.
            - ``baudrate`` – serial baudrate.
            - ``target_positions`` – current target positions.
        """
        with self._lock:
            return {
                "finger_names": list(self._FINGER_NAMES),
    ...
    def initialize(self) -> None:
        ...
        self._start_loop()
        self._logger.info(
            f"RuiyanHand initialised on {self._port} "
    ...
    def shutdown(self) -> None:
        ...
        if self._link is not None:
            ...
            self._link = None
        self._logger.info("RuiyanHand connection closed.")
