...
class ManiskillEnv(gym.Env):
    ...
    def _init_reset_state_ids(self):
        ...
        self.all_reset_state_ids = torch.randperm(self.total_num_group_envs, generator=self._generator).to(self.device)
        self.update_reset_state_ids()

    def update_reset_state_ids(self):
        reset_state_ids = torch.randint(low=0, high=len(self.all_reset_state_ids), size=(self.num_group,), generator=self._generator)
    ...
    def _calc_step_reward(self, info):
        reward = torch.zeros(self.num_envs, dtype=torch.float32).to(self.env.device)  # [B, ]
    ...
    def step(self, actions: Union[Array, Dict] = None, auto_reset=True) -> Tuple[Array, Array, Array, Array, Dict]:
        if actions is None:
        ...
        if self.is_start:
            ...
            terminations = extract_termination_from_info(infos, num_envs=self.num_envs, device=self.device)
    ...
    def run(self):
        ...
        for step in range(100):
            ...
            print(f"Step {step}: obs={obs.keys()}, rew={rew.mean()}, terminations={terminations.float().mean()}, truncations={truncations.float().mean()}")
