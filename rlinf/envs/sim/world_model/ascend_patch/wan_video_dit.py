...
``Patcher`` in ``WanBackend._build_pipeline``.  On Ascend NPU the mindiesd
...
def flash_attention(
    ...
    elif MINDIESD_ENABLE and q.device.type == "npu":
        if q.shape[1] < 4000:
...
def rope_apply(x: torch.Tensor, freqs: torch.Tensor, num_heads: int) -> torch.Tensor:
    """Apply interleaved rotary embeddings to packed Wan attention inputs."""
    if MINDIESD_ENABLE and x.device.type == "npu":
        x = rearrange(x, "b s (n d) -> b s n d", n=num_heads)
...
class RMSNorm(nn.Module):
    """Wan RMS normalization with a fused Ascend implementation."""
