...
class KeyboardListener:
    """Read operator commands from a control file or Linux input device.

    ``RLINF_KEYBOARD_CONTROL_FILE`` selects an append-and-consume text file for
    hosts such as WSL that do not expose ``/dev/input``. Without it, the
    listener reads a physical keyboard through evdev.
    """
    ...
    def __init__(self) -> None:
        ...
        if self._control_file is not None:
            try:
                self._control_file.touch(exist_ok=True)
            except OSError as exc:
                raise RuntimeError(
                    "KeyboardListener could not create the control file set by "
                    f"RLINF_KEYBOARD_CONTROL_FILE='{self._control_file}': {exc}"
                ) from exc
            return
    ...
    def _read_control_file(self) -> list[str]:
        """Consume commands appended to the configured control file."""
        if self._control_file is None:
            return []
        import fcntl

        try:
            with self._control_file.open("r+", encoding="utf-8") as file_obj:
                fcntl.flock(file_obj.fileno(), fcntl.LOCK_EX)
                commands = [line.strip() for line in file_obj if line.strip()]
                file_obj.seek(0)
                file_obj.truncate()
        except FileNotFoundError:
            return []
        normalized = [command.lower() for command in commands]
        return [self.CONTROL_ALIASES.get(command, command) for command in normalized]
    ...
    def get_key(self) -> str | None:
        ...
        """
        if self._control_file is not None:
            commands = self._read_control_file()
    ...
    def pop_pressed_keys(self) -> list[str]:
        ...
        """
        if self._control_file is not None:
            return self._read_control_file()
        with self.state_lock:
