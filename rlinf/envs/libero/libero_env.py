...
class LiberoEnv(gym.Env):
    def __init__(self, cfg, num_envs, seed_offset, total_num_processes, worker_info):
        ...
        # Exhausted eval slots are a FastWAM-specific requirement. Keep the
        # historical LIBERO evaluation path unchanged unless a config opts in.
        self._stop_when_eval_exhausted = bool(
            self.is_eval and cfg.get("stop_when_eval_exhausted", False)
        )
    ...
    def reset(
        ...
        if self.is_start:
            if self.is_eval:
                ...
                self._eval_reset_pool = pool[pool >= 0].copy()
                self.update_reset_state_ids()
                if self._stop_when_eval_exhausted:
    ...
    def _handle_eval_auto_reset(self, dones, _final_obs, infos):
        ...
        if self._stop_when_eval_exhausted:
            # Do not step terminal simulators after their assigned fixed reset
            # states are consumed. A fully inactive batch performs no simulator
            # step; EmbodiedEvalRunner itself has a finite configured horizon.
            self._eval_active[env_idx[~valid_mask]] = False
