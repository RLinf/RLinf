...
class ManiskillEnv(BaseManiskillEnv, EnvOffloadMixin):
    def get_state(self) -> bytes:
        ...
        rng_state = {
            ...
            '_batched_main_rng': get_batch_rng_state(self.env.unwrapped._batched_main_rng),
            '_main_seed': self.env.unwrapped._main_seed,
            '_episode_rng': self.env.unwrapped._episode_rng,
            '_batched_episode_rng': get_batch_rng_state(self.env.unwrapped._batched_episode_rng),
        ...
        simulator_state = {
            ...
            'agent_controller_state': recursive_to_device(self.env.unwrapped.agent.controller.get_state(), 'cpu'),
    ...
    def load_state(self, state_buffer: bytes):
        ...
        self.env.unwrapped.scene.px.cuda_articulation_link_data.torch()[:] = physx_state['cuda_articulation_link_data'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_qacc.torch()[:] = physx_state['cuda_articulation_qacc'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_qf.torch()[:] = physx_state['cuda_articulation_qf'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_qpos.torch()[:] = physx_state['cuda_articulation_qpos'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_qvel.torch()[:] = physx_state['cuda_articulation_qvel'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_target_qpos.torch()[:] = physx_state['cuda_articulation_target_qpos'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_articulation_target_qvel.torch()[:] = physx_state['cuda_articulation_target_qvel'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_rigid_body_data.torch()[:] = physx_state['cuda_rigid_body_data'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene.px.cuda_rigid_dynamic_data.torch()[:] = physx_state['cuda_rigid_dynamic_data'].to(self.env.unwrapped.device)
        self.env.unwrapped.scene._gpu_apply_all()
        self.env.unwrapped.scene.px.gpu_update_articulation_kinematics()
        self.env.unwrapped.scene._gpu_fetch_all()
        self.env.unwrapped.scene._gpu_apply_all()
        ...
        self.env.unwrapped._elapsed_steps = state['elapsed_steps'].to(self.env.unwrapped.device)
        self.env.unwrapped.update_obs_space(state['_init_raw_obs'])
        controller_state = recursive_to_device(state['agent_controller_state'], self.env.unwrapped.device)
        ...
        self.env.unwrapped._batched_main_rng = set_batch_rng_state(rng_state['_batched_main_rng'])
        self.env.unwrapped._main_seed = rng_state['_main_seed']
        self.env.unwrapped._episode_rng = rng_state['_episode_rng']
        self.env.unwrapped._batched_episode_rng = set_batch_rng_state(rng_state['_batched_episode_rng'])
        ...
        self.env.unwrapped._orig_single_action_space = action_space_state['_orig_single_action_space']
