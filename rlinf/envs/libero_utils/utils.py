...
def to_tensor(array: Union[Dict, torch.Tensor, np.ndarray, List, Any], device: str = "cpu") -> Union[Dict, torch.Tensor]:
    """
...
def list_of_dict_to_dict_of_list(list_of_dict: List[Dict[str, Any]]) -> Dict[str, List[Any]]:
    """
...
def get_libero_env(task: Any, model_family: str, seed: int = 0, resolution: int = 256) -> Tuple[OffScreenRenderEnv, str]:
    """
...
def resize_image(img: np.ndarray, resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    """
...
def get_libero_wrist_image(obs: Dict[str, np.ndarray], resize_size: Union[int, Tuple[int, int]]) -> np.ndarray:
    """
