...
class EnvManager:
    def __init__(self, cfg, rank, env_cls, enable_offload=False):
        ...
        if enable_offload:
            ...
            if hasattr(offload_module, class_name):
                ...
                raise RuntimeError(f"Environment class {class_name} does not support offload")
            self.env = None
        else:
    ...
    def start_simulator(self):
        ...
        self.process = self.context.Process(
            ...
            args=(self.cfg, self.rank, self.env_cls, self.command_queue, self.result_queue, self.state_buffer, True)
        )
        self.process.start()
