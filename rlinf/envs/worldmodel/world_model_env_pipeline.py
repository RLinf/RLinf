...
class EvacEnv:
    def __init__(self, cfg, seed_offset, total_num_processes, record_metrics=True):
        ...
        self.dataset = NpyTrajectoryDatasetWrapper(self.cfg.initial_image_path)
    ...
    def _load_action_predictor(self):
        from rlinf.envs.worldmodel.evac.action_model.action_predictor import (
            ActionPredictorMLP,
        )
    ...
    def _record_metrics(self, step_reward, infos):
        ...
        # episode_info["episode_len"] = torch.tensor(self.elapsed_steps)
    ...
    def _infer_next_chunk_frames(self):
        ...
        # when current_step=0, T=n_previous ; when current_step!=0, T=chunk+n_previous
        ...
        # batch = {
        #     "video": video.to(dtype=self.inference_dtype,),
        #     "traj": traj.to(dtype=self.inference_dtype, ),
        #     "delta_action": i_delta_action.to(
        #         dtype=self.inference_dtype,
        #     ),
        #     "domain_id": domain_id,
        #     "intrinsic": intrinsic_batch.to(dtype=torch.float32),
        #     "extrinsic": extrinsic_batch.to(dtype=torch.float32),
        #     "caption": [""],
        #     "cond_id": torch.tensor(
        #         [-self.n_previous - self.chunk], dtype=torch.int64
        #     ),
        ...
        pre_z = None
        pre_img_emb = None
        if self.current_step != 0 and self.all_samples is not None:
        ...
        # Note: current_step is incremented in chunk_step method, not here
