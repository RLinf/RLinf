...
from rlinf.envs.worldmodel.evac.lvdm.common import extract_into_tensor, noise_like
...
# from rlinf.envs.worldmodel.evac.lvdm.models.utils_diffusion import make_ddim_sampling_parameters, make_ddim_timesteps, rescale_noise_cfg
# from rlinf.envs.worldmodel.evac.lvdm.common import noise_like
# from rlinf.envs.worldmodel.evac.lvdm.common import extract_into_tensor


class DDIMSampler(object):
    def __init__(
        self, model, schedule="linear", num_chunk=4, dtype=torch.float32, **kwargs
    ):
        super().__init__()
        self.model = model
    ...
    def make_schedule(
        ...
        def to_torch(x):
            return x.clone().detach().to(self.dtype).to(self.model.device)
        ...
        self.register_buffer(
            ...
            to_torch(torch.log(1.0 - alphas_cumprod.cpu())),
        )
        self.register_buffer(
    ...
    def sample(
        ...
        **kwargs,
        ...
        outputs = self.ddim_sampling_causal(
            conditioning,
            size,
            callback=callback,
    ...
    def ddim_sampling_causal(
        ...
        **kwargs,
        ...
        img_out = [
            img_0,
        ]  # previous frame show red channel
        ...
        for fstep in tqdm(range(total_frames // chunk)):
            ...
            if sparse:
                ...
                cond["c_concat"][0] = torch.cat(
                    [
                        cond["c_concat"][0][:, :, 1:-chunk],
                        add2mem,
                        add2mem.repeat(1, 1, chunk, 1, 1),
                    ],
                    dim=2,
                )  # condition on the generated frames
