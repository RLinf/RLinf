...
def closest_timestamp(**kwargs: Any) -> List[int]:
    ...
    closest_idx = torch.argmin(torch.abs(kwargs["episode_timestamps"] - target_timestamp))
...
class LeRobotDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self._lerobot_dataset = LeRobotDataset(repo_id, root, image_transforms=image_transforms)
        self.timestamps = torch.stack(self._lerobot_dataset.hf_dataset["timestamp"]).numpy()
        ...
        assert (
            start_select_policy in FUNC_MAPPING.keys()
        ), f"start_select_policy {start_select_policy} not in {FUNC_MAPPING.keys()}"
        assert (
            target_select_policy in FUNC_MAPPING.keys()
    ...
    def _get_frame_indices(self, index: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        return torch.arange(start_index, end_index), self.timestamps[start_index:end_index]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--repo_id", type=str, default="unitreerobotics/G1_Dex1_MountCameraRedGripper_Dataset")
    parser.add_argument(
        "--root",
        type=str,
        default="",
