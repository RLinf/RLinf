...
class AsyncOnlineSearchClient:
    ...
    async def _do_jina_access(self, session, url: str) -> Dict:
        ...
        async with session.get(f'https://r.jina.ai/{url}', headers=headers, timeout=aiohttp.ClientTimeout(total=200)) as response:
            if response.status == 200:
...
class WideSeekR1ToolWorker(ToolWorker):
    def __init__(self, cfg: DictConfig):
        ...
        if self.use_online_search:
            self.log_info("[INFO] WideSeekR1ToolWorker: Using online search (Serper API)")
    ...
    async def _process_requests(self):
        ...
        async def generate_and_send(channel_key: str, tool_name: str, tool_args: dict):
            ...
            try:
                ...
                self.log_error(f"[ERROR] WideSeekR1ToolWorker: Tool execution failed for {tool_name}: {e}, tool name is {tool_name}, tool args is {tool_args}")
                result = ToolChannelResponse(
                    success=False,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--is_online', type=lambda x: x.lower() == 'true',
                       default=False,
    ...
    async def test_search_and_access(is_online: bool):
        ...
        print(f"[INFO] Creating search client...")
        ...
        try:
            ...
            for i, result in enumerate(results):
                ...
                if result.get('documents'):
                    print(f"    First document preview: {result['documents'][0][:200] if result['documents'] else 'N/A'}...")
                if result.get('urls'):
                    print(f"    First URL: {result['urls'][0] if result['urls'] else 'N/A'}")
        except Exception as e:
    ...
    # Run the test
    print("=" * 60)
    print(f"Testing WideSeekR1ToolWorker - {'ONLINE' if args.is_online else 'OFFLINE'} mode")
    print("=" * 60)
