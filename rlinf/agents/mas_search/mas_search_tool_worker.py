...
class MasSearchToolWorker(ToolWorker):
    """
    Search tool worker for mas-search, based on SearchToolWorker
    but adapted for multiturn_demo component structure.
    """
    ...
    async def _process_requests(self):
        ...
        while True:
            ...
            assert request.request_type == "execute", (
                "MasSearchToolWorker has no session, so only get 'execute' request_type"
            )
            assert request.tool_name == "search", (
                "MasSearchToolWorker only execute tool_name 'search'"
            )
            asyncio.create_task(
