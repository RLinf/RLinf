...
from rlinf.utils.ckpt_convertor.openpi import sft2new
...
def test_sft2new_uses_robotwin_pi0_dataconfig_and_explicit_fp32(monkeypatch, tmp_path):
    monkeypatch.setattr(
        sft2new,
...
def test_sft2new_uses_behavior_dataconfig_and_explicit_bf16(monkeypatch, tmp_path):
    monkeypatch.setattr(
        sft2new,
        "_get_openpi_train_config",
        lambda config_name: _train_config(
...
def test_sft2new_cli_requires_shared_config_name_and_dtype():
    ...
    assert args._run is sft2new.run


def test_sft2new_cli_rejects_an_implicit_storage_dtype():
