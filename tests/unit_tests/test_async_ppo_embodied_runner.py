...
def test_embodied_runner_initializes_reward_at_configured_output_step(monkeypatch):
    monkeypatch.setattr(
        embodied_runner_module.Channel,
        "create",
        staticmethod(lambda name: _FakeChannel(name)),
    )
    runner = object.__new__(EmbodiedRunner)
    runner.reward = object()
    runner.reward_initialized = False
    runner.reward_channel = None
    runner.global_step = 0
    runner.cfg = OmegaConf.create({"reward": {"use_output_step": 0}})
    runner.logger = type("_Logger", (), {"info": lambda *args, **kwargs: None})()

    EmbodiedRunner._maybe_initialize_reward(runner)
    ...
    runner.reward_channel = None
    runner.global_step = 0
    runner.cfg = OmegaConf.create({"reward": {"use_output_step": 1}})

    EmbodiedRunner._maybe_initialize_reward(runner)

    assert runner.reward_initialized is False
    assert runner.reward_channel is None
