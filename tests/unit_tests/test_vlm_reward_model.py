...
def test_qwentrend_reward_yaml_uses_canonical_backends():
    ...
    assert e2e_cfg.reward.model.inference_backend == "hf"
    assert resolve_reward_model_backend(
        e2e_cfg.reward.model.model_type,
        e2e_cfg.reward.model.inference_backend,
    ) == ("history_vlm", "hf")
...
class _FakeServerWorker:
    ...
    launcher = _FakeWorkerLauncher(_FakeWorkerGroup("server"))

    @classmethod
    def create_group(cls, **kwargs):
        cls.create_group_kwargs = kwargs
        cls.launcher = _FakeWorkerLauncher(_FakeWorkerGroup("server"))
        return cls.launcher


class _FakeRouterWorker:
    create_group_kwargs = None
    launcher = _FakeWorkerLauncher(_FakeWorkerGroup("router"))

    @classmethod
    def create_group(cls, **kwargs):
        cls.create_group_kwargs = kwargs
        cls.launcher = _FakeWorkerLauncher(_FakeWorkerGroup("router"))
        return cls.launcher
...
def test_sglang_reward_server_stack_cleans_up_on_registration_failure(monkeypatch):
    class _FailingRouterGroup(_FakeWorkerGroup):
        def register_server(self, server_url, **kwargs):
            del server_url
            del kwargs
            raise RuntimeError("register failed")

    class _FailingRouterWorker(_FakeRouterWorker):
        @classmethod
        def create_group(cls, **kwargs):
            cls.create_group_kwargs = kwargs
            cls.launcher = _FakeWorkerLauncher(_FailingRouterGroup("router"))
            return cls.launcher
    ...
    monkeypatch.setattr(
        "rlinf.runners.sglang_reward_server._load_sglang_server_worker_classes",
        lambda: (_FakeServerWorker, _FailingRouterWorker),
    )
    ...
    assert _FailingRouterWorker.launcher.group.shutdown_calls == 1
    assert _FakeServerWorker.launcher.group.shutdown_calls == 1
...
def test_qwentrend_sglang_reward_yaml_uses_dedicated_config():
    ...
    e2e_sglang_cfg = OmegaConf.load(
        REPO_ROOT
        / "tests/e2e_tests/embodied/maniskill_ppo_mlp_qwentrend_sglang_reward.yaml"
    )

    assert hf_cfg.reward.model.inference_backend == "hf"
    assert sglang_cfg.reward.model.inference_backend == "sglang"
    assert e2e_sglang_cfg.reward.model.inference_backend == "sglang"
    assert sglang_cfg.cluster.component_placement.reward == "0-0"
...
def test_sglang_reward_server_should_launch_without_api_base():
    cfg = OmegaConf.create(
        {
            "reward": {
                "use_reward_model": True,
    ...
    assert should_launch_sglang_reward_server(cfg)
