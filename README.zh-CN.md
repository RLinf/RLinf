...
RLinf 将硬件差异收敛在同一层加速卡抽象之下，同一套训练栈可以运行在 NVIDIA、AMD GPU 以及华为昇腾、摩尔线程、昆仑芯等国产加速卡上。用户可以在不同加速卡上训练，模型、算法与配置都无需改动。加速卡厂商只需实现一层很小的接口，就能把完整的具身与智能体 RL 训练栈带到自家硬件上，而不必维护框架的分支。
