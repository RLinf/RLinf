...
from rlinf.agents.mas.mas_agent_loop_tool import MASToolAgentLoopWorker_Tool_Parallel
from rlinf.agents.tools.mas_search_tool_worker import MASToolWorker

from rlinf.config import validate_cfg
from rlinf.data.datasets import create_rl_dataset
from rlinf.data.tokenizers import hf_tokenizer

from rlinf.runners.mas_tool_agent_eval_runner import MASToolAgentRunnerEval
...
def main(cfg) -> None:
    ...
    if cfg.rollout.get('use_sub_worker', False):    
        # main agent and sub agent use different rollout engine
        rollout_accel_num = rollout_placement_strategy._end_accel_id - rollout_placement_strategy._start_accel_id + 1
        assert rollout_accel_num % 2 == 0
        assert (rollout_accel_num // 2) % cfg.rollout.tensor_parallel_size == 0
        assert (rollout_accel_num // 2) % cfg.rollout_sub_worker.tensor_parallel_size == 0
        planner_rollout_placement_strategy = PackedPlacementStrategy(
            rollout_placement_strategy._start_accel_id,
            rollout_placement_strategy._start_accel_id + rollout_accel_num // 2 - 1,
            num_accelerators_per_process=rollout_placement_strategy._num_accelerators_per_process,
            stride=rollout_placement_strategy._stride,
        )
        subwoker_rollout_placement_strategy = PackedPlacementStrategy(
            rollout_placement_strategy._start_accel_id + rollout_accel_num // 2,
            rollout_placement_strategy._end_accel_id,
            num_accelerators_per_process=rollout_placement_strategy._num_accelerators_per_process,
            stride=rollout_placement_strategy._stride,
        )
        ...
        subworker_rollout_group = rollout_worker_cls.create_group(
            cfg,
            component_placement, 
            config_rollout=cfg.rollout_sub_worker,
            sampling_params=None,
        ).launch(
            cluster,
            name=cfg.rollout_sub_worker.group_name,
    ...
    agentloop_group = MASToolAgentLoopWorker_Tool_Parallel.create_group(cfg, component_placement).launch(
        cluster,
    ...
    train_ds, val_ds = create_rl_dataset(cfg, tokenizer, is_eval=True)
    ...
    tool_workers = {
        MASToolWorker.create_group(cfg).launch(
    ...
    runner = MASToolAgentRunnerEval(
        ...
        solid_rollouts={"subworker": subworker_rollout_group} if subworker_rollout_group is not None else {}
    )
