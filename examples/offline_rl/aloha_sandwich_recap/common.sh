...
POLICY_CKPT="${POLICY_CKPT:-/inspire/qb-ilm/project/robot-reasoning/czxs253130583/yushun/openpi/checkpoints/torch/pi05_sandwich_new_all_rlinf_base}"
...
export REPO CONFIG_DIR PY RAW_DATA DATASET POLICY_CKPT RUN_ROOT
...
export PYTHONPATH="${REPO}:${LIBERO_REPO_PATH:-}:${PYTHONPATH:-}"
...
require_policy_checkpoint() {
  if [[ ! -d "${POLICY_CKPT}" ]]; then
    log "ERROR policy checkpoint directory missing: ${POLICY_CKPT}"
    exit 1
  fi
}
