...
def load_dataset(dataset_path: Path) -> Tuple[LeRobotDataset, LeRobotDatasetMetadata, Dict]:
...
def detect_image_keys(sample: Dict) -> List[str]:
    ...
    for key in sample.keys():
        ...
        if key.startswith("observation.images.") or key in ["image", "wrist_image", "front_image", "left_image", "right_image"]:
            val = sample[key]
            arr = to_numpy(val)
...
def create_advantage_distribution_plot(
    ...
    fig.suptitle(f"Advantage Analysis: {dataset_path.name}", fontsize=14, fontweight='bold')
    ...
    ax.hist(adv, bins=100, range=(xlim_lo, xlim_hi), density=True, alpha=0.7,
            color='steelblue', edgecolor='black', linewidth=0.5)
    ax.axvline(x=0, color='red', linestyle='--', linewidth=1.5, label='Zero')
    ax.axvline(x=np.mean(adv), color='green', linestyle='-', linewidth=1.5, label=f'Mean={np.mean(adv):.4f}')
    if threshold is not None:
        ax.axvline(x=threshold, color='orange', linestyle='-', linewidth=2.0,
    ...
    if len(df['value_current']) > 0:
        ...
        ax.hist(v_curr, bins=100, density=True, alpha=0.7, color='forestgreen', edgecolor='black', linewidth=0.5)
        ax.set_xlabel('V(o_t)')
        ax.set_ylabel('Density')
        ax.set_title(f'Value Predictions (mean={np.mean(v_curr):.4f})')
        ax.grid(True, alpha=0.3)
    else:
        ax.text(0.5, 0.5, 'No value_current data', ha='center', va='center', transform=ax.transAxes)
    ...
    if 'value_current' in df.columns and len(df['value_current']) > 0:
        ...
        if threshold is not None:
            ax.axhline(y=threshold, color='orange', linestyle='-', linewidth=1.5, label=f'Thresh={threshold:.4f}')
        ax.set_xlabel('V(o_t)')
    ...
    if threshold is not None:
        ...
        ax.bar(ep_pos['episode_index'], ep_pos['positive_rate'] * 100, color='coral', alpha=0.7, edgecolor='black', linewidth=0.3)
        ax.axhline(y=ep_pos['positive_rate'].mean() * 100, color='red', linestyle='--', linewidth=1.5,
        ...
        ax.text(0.5, 0.5, 'No threshold set', ha='center', va='center', transform=ax.transAxes)

    # 5. Advantage by episode
    ax = axes[1, 1]
    ep_stats = df.groupby('episode_index')['advantage_value'].agg(['mean', 'std']).reset_index()
    ax.errorbar(ep_stats['episode_index'], ep_stats['mean'], yerr=ep_stats['std'],
                fmt='o', markersize=3, alpha=0.7, capsize=2, color='teal')
...
def create_episode_video(
    ...
    if has_advantages:
        ...
        if threshold is not None:
            ax_adv.axhline(y=threshold, color='orange', linestyle='-', linewidth=1.5,
    ...
    for ax in camera_axes:
        ...
        overlay = Rectangle(
            (0, 0), 1, 1, transform=ax.transAxes,
            linewidth=0, edgecolor='none', facecolor='lime', alpha=0.15,
    ...
    title = fig.suptitle(f"Episode {episode_data['episode_index']} - {task_text}\nFrame: {frames[0]}", fontsize=11)
    ...
    def update(frame_num):
        ...
        title.set_text(f"Episode {episode_data['episode_index']} - {task_text}\n"
...
def create_episode_summary_plot(
    ...
    sample_indices = sorted(set(i for i in sample_indices if i < n_frames))

    fig = plt.figure(figsize=figsize)
    gs = gridspec.GridSpec(n_cameras + 2, len(sample_indices), figure=fig,
    ...
    for cam_idx, key in enumerate(image_keys):
        ...
        for col_idx, frame_idx in enumerate(sample_indices):
            ...
            if col_idx == 0:
                ax.text(-0.1, 0.5, cam_name, transform=ax.transAxes, fontsize=9,
    ...
    if threshold is not None:
        ...
        ax_adv.fill_between(frames, adv_arr_plot, threshold,
                            where=above_mask, alpha=0.2, color='lime',
    ...
    fig.suptitle(f"Episode {episode_data['episode_index']}: {task_text}\n"
                 f"Advantage: mean={adv_arr.mean():.4f}, std={adv_arr.std():.4f}", fontsize=10)
...
def main():
    parser = argparse.ArgumentParser(description="Visualize advantage distribution and episodes")
    parser.add_argument("--dataset", type=str, required=True, help="Path to LeRobot dataset")
    parser.add_argument("--output", type=str, default="outputs/advantage_viz", help="Output directory")
    parser.add_argument("--episodes", type=int, nargs="+", help="Specific episode indices to visualize")
    parser.add_argument("--num-episodes", type=int, default=10, help="Number of episodes (0=all)")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for sampling")
    parser.add_argument("--fps", type=int, default=10, help="Video FPS")
    parser.add_argument("--no-video", action="store_true", help="Skip video generation")
    parser.add_argument("--no-distribution", action="store_true", help="Skip distribution plot")
    parser.add_argument("--advantage-key", type=str, default="advantage_continuous",
    ...
    if not args.no_distribution:
        ...
        create_advantage_distribution_plot(dataset_path, dist_path, threshold=threshold,
    ...
    if args.episodes:
        ...
        if args.num_episodes <= 0:
            ...
            episode_indices = np.random.choice(all_episodes, min(args.num_episodes, len(all_episodes)), replace=False)
